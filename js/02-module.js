...
function scrollChatSmart(box){
  ...
  const visitorReading = isVisitorView && (
    (window.__tomaunoVisitorReadingUntil && Date.now() < window.__tomaunoVisitorReadingUntil) ||
    distanceFromBottom > 90
  );
  if(visitorReading) return;
...
window.toggleModoAsistenteChat = async () => {
  ...
  const oldTop = box ? box.scrollTop : null;
  ...
  if(currentOpenChatId && nuevo === 'automatico'){
    try{ await update(ref(db,'tomauno/chats/'+currentOpenChatId), {javierOnline:false, javierOnlineAt:0, humanMode:false}); }catch(e){}
  }else if(currentOpenChatId && nuevo === 'manual'){
    try{ await update(ref(db,'tomauno/chats/'+currentOpenChatId), {javierOnline:true, javierOnlineAt:Date.now(), humanMode:true}); }catch(e){}
  }
  ...
  setTimeout(()=>{ const b=document.getElementById('chat-msgs'); if(b && oldTop !== null) b.scrollTop = oldTop; }, 180);
};
...
// ─────────────────────────────────────────────────────────────────────────────
// TOMAUNO v28b FINAL 2 — SIN DUPLICAR HUMANO
// Base: v28b. NO reemplaza flujo humano original.
// Corrige solo Enter, altura, scroll, amarillo leído y notificaciones de usuario.
// ─────────────────────────────────────────────────────────────────────────────
(function(){
  'use strict';

  function safe(fn){ try{return fn();}catch(e){ try{console.warn('TU final2:',e);}catch(_){} } }
  ...
  // 1) ENTER visitante definitivo: envía una sola vez y no deja que listeners viejos lo coman.
  var sending = false;
  var last = {id:'',text:'',at:0};
  ...
  async function sendVisitor(){
    ...
    if(!id || !text) return;
    if(last.id === id && last.text === text && Date.now() - last.at < 1200) return;

    last = {id:id,text:text,at:Date.now()};
    sending = true;
    ...
    setTimeout(function(){ scrollVisitorToAnswer(true); },320);
    setTimeout(function(){ scrollVisitorToAnswer(true); },950);

    sending = false;
  }

  window.enviarChatVisitante = sendVisitor;
  try{ enviarChatVisitante = sendVisitor; }catch(e){}
  ...
  // 2) Alto visitante / móvil sin tocar admin.
  function applyLayout(){
    var pop = q('#chat-popover.open');
    if(!pop) return;

    var admin = isAdmin();
    pop.classList.toggle('tu-final2-visitor',!admin);

    var mobile = !admin && window.matchMedia && window.matchMedia('(max-width:700px)').matches;
    pop.classList.toggle('tu-final2-mobile',!!mobile);
    document.body.classList.toggle('tu-final2-mobile-open',!!mobile);
    document.documentElement.classList.toggle('tu-final2-mobile-open',!!mobile);
  }

  // 3) Scroll visitante al inicio de respuesta, pero libera si usuario toca scroll.
  ...
  function scrollVisitorToAnswer(force){
    ...
    var lastAdmin = admins[admins.length-1];

    requestAnimationFrame(function(){
      box.scrollTop = Math.max(0,lastAdmin.offsetTop - 18);
    });
  }
  ...
  function watchAnswer(){
    ...
    var lastAdmin = admins[admins.length-1];
    var key = String(lastAdmin.innerText || '').slice(0,180) + '|' + String(lastAdmin.innerText || '').length;

    if(key && key !== lastAnswerKey){
      lastAnswerKey = key;
      setTimeout(function(){ scrollVisitorToAnswer(false); },150);
    }
  }

  // 4) Amarillo leído: limpiar visual + local + Firebase.
  ...
  if(typeof oldOpenAdmin === 'function' && !oldOpenAdmin.__tuFinal2Read){
    var wrappedOpen = function(id){
      ...
      var r = oldOpenAdmin.apply(this,arguments);

      setTimeout(function(){ if(id) clearUnreadVisual(id); },160);
      setTimeout(function(){ if(id) clearUnreadVisual(id); },650);

      return r;
    };

    wrappedOpen.__tuFinal2Read = 1;
    window.abrirChatAdmin = wrappedOpen;
    try{ abrirChatAdmin = wrappedOpen; }catch(e){}
  }

  // Si el chat está abierto, mantener leído.
  function keepOpenChatRead(){
    if(!isAdmin()) return;
    var id = adminId();
    if(!id) return;

    setLocal(id,{unreadAdmin:false});
    clearUnreadVisual(id);
  }

  // 5) Al responder admin limpia humano/amarillo.
  var oldSendAdmin = window.enviarChatAdmin;
  if(typeof oldSendAdmin === 'function' && !oldSendAdmin.__tuFinal2Send){
    var wrappedSend = async function(){
      var id = adminId();
    ...
    wrappedSend.__tuFinal2Send = 1;
    window.enviarChatAdmin = wrappedSend;
    try{ enviarChatAdmin = wrappedSend; }catch(e){}
  }

  // 6) Notificaciones: solo primer mensaje de usuario. No mensajes del asistente/admin.
  var notified = {};

  function firstUserMsg(chat){
  ...
  function sanitizeNotificationDom(){
    // Oculta toasts que claramente son textos del asistente, no del usuario.
    qa('body *').slice(-60).forEach(function(el){
      var t = el.innerText || '';
      if(!/CHAT NUEVO MENSAJE WEB/i.test(t)) return;
      if(/Javier puede estar ocupado|whatsapp|dejame tu consulta|Llamando a Javier|Muchas gracias|Para dejarle tu consulta|Como profesor|Director actualmente/i.test(t)){
        el.style.display = 'none';
      }
    });
  }

  function notifyOnlyFirstUser(){
    if(!isAdmin()) return;
    ...
    Object.entries(chats()).forEach(function(pair){
      ...
      try{
        ...
        if(AC){
          ...
          setTimeout(function(){ if(ctx.close) ctx.close(); },650);
        }
      }catch(e){}
    });
  }

  // 7) Mantener flujo humano original de v28b; solo mejoramos diseño del contador existente.
  function prettifyHumanCounter(){
    qa('#chat-popover.open .chat-bubble.admin').forEach(function(b){
      var t = b.innerText || '';
      if(/Llamando a Javier|intentar conectar con Javier|Javier responderá personalmente/i.test(t)){
        b.classList.add('tu-final2-human-bubble');
      }
    });
  }

  // 8) Typing visible en admin.
  function keepTypingVisible(){
    if(!isAdmin()) return;
  ...
  // 9) Evitar cierre al usar scroll/lista admin.
  ['pointerdown','mousedown','touchstart','wheel'].forEach(function(ev){
    document.addEventListener(ev,function(e){
      if(e.target && e.target.closest && e.target.closest('#chat-popover.open .chat-tabs,#chat-popover.open .chat-msgs,.chat-inbox-side,.chat-list')){
        e.stopPropagation();
      }
    },true);
  });
  ...
  function css(){
    ...
    st.textContent = `
      @media(min-width:701px){
        #chat-popover.open.tu-final2-visitor,
        body:not(.tomauno-admin-active) #chat-popover.open{
      ...
      @media(max-width:700px){
        html.tu-final2-mobile-open,
        body.tu-final2-mobile-open{
          overflow:hidden!important;
          height:100%!important;
        }

        #chat-popover.open.tu-final2-mobile{
          position:fixed!important;
          left:0!important;
          right:0!important;
          top:0!important;
        ...
        #chat-popover.open.tu-final2-mobile .chat-panel{
          height:100%!important;
          min-height:0!important;
          display:flex!important;
          flex-direction:column!important;
        ...
        #chat-popover.open.tu-final2-mobile .chat-msgs{
          flex:1 1 auto!important;
          min-height:0!important;
          max-height:none!important;
          height:auto!important;
        ...
        #chat-popover.open.tu-final2-mobile .chat-row{
          display:grid!important;
          grid-template-columns:1fr 58px!important;
          gap:8px!important;
          flex:0 0 auto!important;
        ...
        #chat-popover.open.tu-final2-mobile #chat-text{
          height:56px!important;
          min-height:56px!important;
          font-size:16px!important;
          padding:0 16px!important;
        ...
        #chat-popover.open.tu-final2-mobile .chat-send{
          width:56px!important;
          height:56px!important;
          min-width:56px!important;
          border-radius:50%!important;
        ...
        body.tu-final2-mobile-open #chat-fab{
          display:none!important;
          pointer-events:none!important;
        }
      }
      ...
      #chat-popover.open .tu-final2-human-bubble{
        border-radius:22px!important;
      }

      #chat-popover.open .tu-final2-human-bubble b,
      #chat-popover.open .tu-final2-human-bubble strong{
        color:#fff!important;
      }
    `;
  ...
  setInterval(function(){
    ...
    keepOpenChatRead();
    notifyOnlyFirstUser();
    sanitizeNotificationDom();
    prettifyHumanCounter();
    keepTypingVisible();
  },700);
})();
...
// ─────────────────────────────────────────────────────────────────────────────
// TOMAUNO v28b FINAL 3 — HUMANO ESTABLE + SIN ACTIVAR ALERTAS
// No reconstruye admin. Corrige restos de alerta, humano, amarillo y mensajes auto/manual.
// ─────────────────────────────────────────────────────────────────────────────
(function(){
  'use strict';

  function safe(fn){ try{return fn();}catch(e){ try{console.warn('TU final3:',e);}catch(_){} } }
  function q(s,r){ return (r||document).querySelector(s); }
  function qa(s,r){ return Array.from((r||document).querySelectorAll(s)); }
  function now(){ return Date.now(); }
  function norm(s){
  ...
  // 1) Quitar cualquier botón viejo de activar alertas/llamadas.
  function removeAlertButtons(){
    qa('.tu-v28d-sound-unlock,.tu-call-sound-unlock,.tu-sound-unlock,.tu-v28d-sound-unlock,.tu-v34-sound-unlock').forEach(function(n){
      n.remove();
    });

    qa('button').forEach(function(b){
      var t = b.innerText || '';
      if(/activar alertas|activar llamada|activar llamadas/i.test(t)){
        b.remove();
      }
    });
  }

  // 2) Saber modo del asistente.
  function isAutoMode(){
    return safe(function(){ return asistenteModo() === 'automatico'; }) !== false;
  }

  // 3) Humano con llamada real cada 10s durante 60s.
  function isHumanRequest(text){
    var x = norm(text);
    return [
  ...
  function humanCallText(sec){
    ...
    if(isAutoMode()){
      return '📞 Llamando a Javier... '+String(sec).padStart(2,'0')+'s\n\nSi no llega a responder en este momento, dejame tu WhatsApp y tu consulta para que pueda responderte apenas quede libre.';
    }
    return '🟢 Javier está en línea.\n\nEscribí tu consulta y te responde por acá.';
  }

  function phoneDetected(text){
  ...
  function ringSound(){
    try{
      ...
      var ctx = new AC();
      var t = ctx.currentTime;
      [720,920,1160,920].forEach(function(f,i){
        var o = ctx.createOscillator();
        var g = ctx.createGain();
        o.type = 'sine';
        o.frequency.value = f;
      ...
      setTimeout(function(){ if(ctx.close) ctx.close(); },800);
    }catch(e){}
  }

  function startRing(id){
    if(!id || ringTimers[id]) return;
    ringStartedAt[id] = Date.now();

    function tick(){
      if(!ringTimers[id]) return;
      if(Date.now() - ringStartedAt[id] > 60000){
        stopRing(id);
        return;
      }
  ...
  // Reemplazar responder humano, pero evitar duplicados.
  var oldResponder = window.responderAutomaticoChat;
  if(typeof oldResponder === 'function' && !oldResponder.__tuFinal3Human){
    var wrappedResponder = async function(id,text){
      var chat = chats()[id] || {};

      // Si ya pidió humano y ahora deja teléfono, cerrar correctamente.
      if((chat.humanRequested || chat.waitingHuman || chat.priority) && phoneDetected(text)){
        stopRing(id);
        ...
        setTimeout(function(){ scrollVisitorToAnswer(true); },250);
        return;
      }
      ...
      if(isHumanRequest(text)){
        ...
        // Evitar duplicar mensaje humano si ya existe uno reciente en el chat.
        var already = false;
        try{
          var ms = (chats()[id] || {}).messages || {};
          Object.keys(ms).forEach(function(k){
            var m = ms[k] || {};
            if(m.from === 'admin' && /Llamando a Javier|Javier responderá personalmente|Javier está en línea/i.test(String(m.text || '')) && Date.now() - Number(m.createdAt || 0) < 120000){
              already = true;
            }
          });
        }catch(e){}
    ...
    wrappedResponder.__tuFinal3Human = 1;
  ...
  function updateCountdown(){
    ...
    qa('#chat-popover.open .chat-bubble.admin').forEach(function(b){
      if(/Llamando a Javier|Javier responderá personalmente|Javier está en línea/i.test(b.innerText || '')){
        b.innerHTML = '<div class="tu-final3-call">'+humanCallText(left).replace(/\n/g,'<br>')+'</div><div class="chat-meta">Ahora</div>';
      }
    });
  }

  // 5) Amarillo/rojo: abrir chat apaga amarillo, responder apaga rojo.
  function clearUnreadVisual(id){
    qa('.chat-tab,.chat-list-item,.chat-inbox-item,[data-chat-id]').forEach(function(el){
      var cid = el.getAttribute('data-chat-id') || el.dataset.chatId || '';
      var onclick = el.getAttribute('onclick') || '';
  ...
  var oldOpenAdmin = window.abrirChatAdmin;
  if(typeof oldOpenAdmin === 'function' && !oldOpenAdmin.__tuFinal3Read){
    var wrappedOpen = function(id){
      if(id){
        setLocal(id,{unreadAdmin:false});
        updateChat(id,{unreadAdmin:false,readByAdminAt:Date.now()});
        clearUnreadVisual(id);
      }
      ...
      setTimeout(function(){ if(id) clearUnreadVisual(id); },160);
      setTimeout(function(){ if(id) clearUnreadVisual(id); },700);

      return r;
    };

    wrappedOpen.__tuFinal3Read = 1;
    window.abrirChatAdmin = wrappedOpen;
    try{ abrirChatAdmin = wrappedOpen; }catch(e){}
  }

  var oldSendAdmin = window.enviarChatAdmin;
  if(typeof oldSendAdmin === 'function' && !oldSendAdmin.__tuFinal3Send){
    var wrappedSend = async function(){
      var id = adminId();
      var r = await oldSendAdmin.apply(this,arguments);
    ...
    wrappedSend.__tuFinal3Send = 1;
    window.enviarChatAdmin = wrappedSend;
    try{ enviarChatAdmin = wrappedSend; }catch(e){}
  }

  // Mantener chat abierto leído cada tanto.
  function keepRead(){
    if(!isAdmin()) return;
    var id = adminId();
    if(!id) return;
  ...
  // 6) Evitar mostrar bandeja antes del chat si ya hay chat seleccionado.
  function reduceInboxFlash(){
    if(!isAdmin()) return;
    var id = adminId();
    if(!id) return;

    var title = q('#chat-popover.open .chat-title');
    var inboxOnly = /BANDEJA DE CHATS|BANDEJA/i.test(title ? title.innerText || '' : '');

    if(inboxOnly && typeof window.abrirChatAdmin === 'function'){
      setTimeout(function(){
        try{ window.abrirChatAdmin(id,true); }catch(e){}
      },80);
    }
  }

  // 7) Ocultar notificaciones con texto de asistente.
  function sanitizeNotifications(){
    qa('body *').slice(-80).forEach(function(el){
      var t = el.innerText || '';
      if(!/CHAT NUEVO MENSAJE WEB/i.test(t)) return;

      if(/Javier puede estar ocupado|whatsapp|dejame tu consulta|Llamando a Javier|Muchas gracias|Para dejarle tu consulta|Como profesor|Director actualmente|Javier responderá/i.test(t)){
        el.style.display = 'none';
      }
    });
  }

  // 8) Mensaje del visitante según auto/manual.
...
// ─────────────────────────────────────────────────────────────────────────────
// TOMAUNO v28b FINAL 4 — LÓGICA CHAT/SONIDO/AMARILLO/CEREBRO
// No toca layout. Corrige alarma, amarillo leído, alquiler/cerebro,
// cierre del contador y Javier online al tomar humano.
// ─────────────────────────────────────────────────────────────────────────────
(function(){
  'use strict';

  function safe(fn){ try{return fn();}catch(e){ try{console.warn('TU final4:',e);}catch(_){} } }
  function q(s,r){ return (r||document).querySelector(s); }
  function qa(s,r){ return Array.from((r||document).querySelectorAll(s)); }
  function now(){ return Date.now(); }
  function norm(s){
  ...
  // 1) Quitar botón visible "Activar alertas/llamadas".
  function removeAlertButtons(){
    qa('.tu-v28d-sound-unlock,.tu-call-sound-unlock,.tu-sound-unlock,.tu-v34-sound-unlock').forEach(function(n){ n.remove(); });
    qa('button').forEach(function(b){
      if(/activar alertas|activar llamada|activar llamadas|activar sonido/i.test(b.innerText || '')) b.remove();
    });
  }

  // 2) Audio: sin botón. Se desbloquea con cualquier interacción admin.
  var audioCtx = null;
  ...
  function unlockAudio(){
    ...
    safe(function(){
      ...
      audioUnlocked = true;
      try{ sessionStorage.setItem('tu_audio_unlocked','1'); }catch(e){}
    });
  }

  document.addEventListener('click',function(){ if(isAdmin()) unlockAudio(); },true);
  document.addEventListener('keydown',function(){ if(isAdmin()) unlockAudio(); },true);
  ...
  function alarm(kind){
    safe(function(){
      ...
      var freqs = kind === 'human' ? [720,920,1160,920] : [880,1120,880];
      ...
      freqs.forEach(function(f,i){
        ...
        o.type = 'sine';
        o.frequency.value = f;
        g.gain.value = kind === 'human' ? 0.24 : 0.16;
        o.connect(g);
        g.connect(ctx.destination);
        o.start(t+i*0.16);
        o.stop(t+i*0.16+0.12);
      });
    });
  }

  // 3) Notificación: solo primer mensaje del usuario por chat. No textos del asistente.
  var notified = {};
  try{ notified = JSON.parse(sessionStorage.getItem('tu_final4_notified') || '{}'); }catch(e){ notified = {}; }

  function saveNotified(){
    try{ sessionStorage.setItem('tu_final4_notified',JSON.stringify(notified)); }catch(e){}
  }
  ...
  function isAssistantText(text){
    return /Javier puede estar ocupado|whatsapp|dejame tu consulta|Llamando a Javier|Muchas gracias|Para dejarle tu consulta|Javier responderá|Como profesor|Director actualmente|Podés tocar|Ver cursos|Ver servicios/i.test(String(text || ''));
  }

  // Blindar notifyAdminChat: si llega texto del asistente, no notificar.
  var oldNotify = window.notifyAdminChat || (typeof notifyAdminChat !== 'undefined' ? notifyAdminChat : null);
  if(typeof oldNotify === 'function' && !oldNotify.__tuFinal4){
    var wrappedNotify = function(title,body,chatId){
      if(isAssistantText(body)) return;
      return oldNotify.apply(this,arguments);
    };
    wrappedNotify.__tuFinal4 = 1;
    window.notifyAdminChat = wrappedNotify;
    try{ notifyAdminChat = wrappedNotify; }catch(e){}
  }

  function notifyNewVisitorOnly(){
    if(!isAdmin()) return;
    ...
    Object.entries(chats()).forEach(function(pair){
      ...
      if(Date.now() - Number(msg.createdAt || 0) > 4*60*1000){
        notified[id] = true;
        saveNotified();
        return;
      }
      ...
      // También usar banner visual propio si existe, con texto del usuario.
      try{
        if(typeof showNotif === 'function') showNotif();
        if(typeof showNotifBanner === 'function'){
          var name = '';
          try{ name = chatVisibleName(chat,id); }catch(e){ name = chat.name || 'Visitante'; }
          showNotifBanner('CHAT NUEVO MENSAJE WEB', name + ': ' + String(msg.text || 'Escribió desde la web'), '💬', function(){
            try{ window.abrirChatAdmin(id); }catch(e){}
          });
        }
      }catch(e){}
    });
  ...
  // 4) Amarillo: es unread/priority/waiting. Al abrir chat, borrar unread y PRIORIDAD visual.
  function clearYellowVisual(id){
    qa('.chat-tab,.chat-list-item,.chat-inbox-item,[data-chat-id]').forEach(function(el){
      var cid = el.getAttribute('data-chat-id') || el.dataset.chatId || '';
      var onclick = el.getAttribute('onclick') || '';
      ...
      el.classList.remove('new','unread','waiting','tu-state-unread','has-new','priority');
      el.style.borderColor = '';
      el.style.boxShadow = '';

      var badge = q('.badge,.tu-state-badge',el);
      if(badge && /(NUEVO|PRIORIDAD|ESPERANDO)/i.test(badge.textContent || '')){
        badge.textContent = '';
      }

      // textos internos "PRIORIDAD / ESPERANDO" que quedan en preview
      qa('*',el).forEach(function(n){
        if(/^(PRIORIDAD|ESPERANDO|NUEVO)$/i.test(String(n.textContent || '').trim())){
          n.textContent = '';
        }
      });
    });
  ...
  if(typeof oldOpen === 'function' && !oldOpen.__tuFinal4Open){
    var wrappedOpen = function(id){
      var c = id ? (chats()[id] || {}) : {};
      var wasHuman = !!(c.humanRequested || c.waitingHuman || c.priority);

      if(id){
        // Abrir = leído. Si era humano, queda humano solicitado, pero sin amarillo/prioridad.
        updateChat(id,{
          unreadAdmin:false,
          waitingHuman:false,
          priority:false,
      ...
      setTimeout(function(){ if(id) clearYellowVisual(id); },150);
      setTimeout(function(){ if(id) clearYellowVisual(id); },800);
      return r;
    };
    wrappedOpen.__tuFinal4Open = 1;
    window.abrirChatAdmin = wrappedOpen;
    try{ abrirChatAdmin = wrappedOpen; }catch(e){}
  }

  var oldSend = window.enviarChatAdmin;
  if(typeof oldSend === 'function' && !oldSend.__tuFinal4Send){
    var wrappedSend = async function(){
      var id = adminId();
    ...
    wrappedSend.__tuFinal4Send = 1;
    window.enviarChatAdmin = wrappedSend;
    try{ enviarChatAdmin = wrappedSend; }catch(e){}
  }

  function keepOpenRead(){
    if(!isAdmin()) return;
    var id = adminId();
    if(!id) return;

    var c = chats()[id] || {};
    if(c.unreadAdmin || c.priority || c.waitingHuman){
      setLocal(id,{unreadAdmin:false,priority:false,waitingHuman:false});
      clearYellowVisual(id);
    }
  }

  // 5) Cerebro: alquiler/estudio/equipos debe priorizar knowledge antes que cursos.
  var oldBuscar = (typeof buscarRespuestaAsistente === 'function') ? buscarRespuestaAsistente : null;
  if(oldBuscar && !oldBuscar.__tuFinal4Brain){
    var wrappedBuscar = function(text){
      var qn = norm(text);
      var isRental = /(alquil|alquiler|alquilan|rentan|rentar|renta|estudio|equipos|luces|fondo|fondos)/.test(qn);

      if(isRental){
        try{
          var matches = typeof knowledgeMatchesAI === 'function' ? knowledgeMatchesAI(qn) : [];
          if(matches && matches.length){
            // Priorizar entradas que tengan las palabras en titulo/keys/command.
            var best = matches.find(function(m){
              var k = m.k || {};
              return /(alquil|alquiler|alquilan|rentan|rentar|estudio|equipos|luces|fondo)/i.test([k.titulo||'',k.keys||'',k.command||''].join(' '));
            }) || matches[0];

            if(best && best.k && best.k.respuesta) return best.k.respuesta;
          }

          if(typeof asistenteKnowledgeEntries === 'function'){
            var entries = asistenteKnowledgeEntries();
            var found = null;
            ...
            entries.forEach(function(pair){
              ...
              if(/alquil|alquiler|alquilan|rentan|rentar|estudio|equipos|luces|fondo/.test(hay)){
                found = k;
              }
            });
      ...
      return oldBuscar.apply(this,arguments);
    };

    wrappedBuscar.__tuFinal4Brain = 1;
    buscarRespuestaAsistente = wrappedBuscar;
    try{ window.buscarRespuestaAsistente = wrappedBuscar; }catch(e){}
  }

  // 6) Humano: ring cada 10s y fallback al llegar a 00.
  var ringTimers = {};
  var fallbackSent = {};
  ...
  function checkHumanTimeout(){
    Object.entries(chats()).forEach(function(pair){
      var id = pair[0];
      var c = pair[1] || {};
      if(!c.callUntil || !(c.humanRequested || c.waitingHuman || c.priority)) return;
      if(fallbackSent[id] || c.humanFallbackSent) return;

      if(Date.now() >= Number(c.callUntil)){
  ...
  // Detectar cuando se activa llamada humana y sonar.
  var seenHuman = {};
  function watchHumanRequests(){
    Object.entries(chats()).forEach(function(pair){
      var id = pair[0];
      var c = pair[1] || {};
      if(!(c.humanRequested || c.waitingHuman || c.priority)) return;
      if(!c.callUntil) return;
  ...
  // 7) Si el usuario ya dejó teléfono luego del fallback, confirmar.
  ...
  var oldResponder = window.responderAutomaticoChat;
  if(typeof oldResponder === 'function' && !oldResponder.__tuFinal4Responder){
    var wrappedResponder = async function(id,text){
      var c = chats()[id] || {};
      ...
      var r = await oldResponder.apply(this,arguments);

      // Si la respuesta anterior activó humano, detectar y sonar.
      setTimeout(watchHumanRequests,300);
      return r;
    };

    wrappedResponder.__tuFinal4Responder = 1;
    window.responderAutomaticoChat = wrappedResponder;
    try{ responderAutomaticoChat = wrappedResponder; }catch(e){}
  }

  function css(){
    if(q('#tu-final4-css')) return;
  ...
  setInterval(function(){
    removeAlertButtons();
    notifyNewVisitorOnly();
    keepOpenRead();
    // final6g: checkHumanTimeout(); desactivado
    // final6g: watchHumanRequests(); desactivado
    // final6e: header visitante estable, sin alternar textos
  },800);
})();
...
// ─────────────────────────────────────────────────────────────────────────────
// TOMAUNO v28b FINAL 5 — SONIDO GLOBAL + ESTADOS + HUMANO
// No toca layout. Corrige:
// - audio desbloqueado desde la web principal/admin, no solo chat
// - amarillo persistente desde DB/localStorage/clases
// - llamada humana cada 10s más audible
// - texto humano actualizado
// - visitante estable: "Javier está en línea"
// - evita responder catálogo si solo dice "mi nombre es X"
// ─────────────────────────────────────────────────────────────────────────────
(function(){
  'use strict';

  function safe(fn){ try{return fn();}catch(e){ try{console.warn('TU final5:',e);}catch(_){} } }
  function q(s,r){ return (r||document).querySelector(s); }
  function qa(s,r){ return Array.from((r||document).querySelectorAll(s)); }
  function now(){ return Date.now(); }
  function norm(s){
  ...
  // 1) Audio global: se desbloquea con cualquier click/tecla en la web principal.
  var audioCtx = null;
  var unlocked = false;
  ...
  function unlock(){
    ...
    safe(function(){
      ...
      unlocked = true;
      try{ sessionStorage.setItem('tu_audio_global_ok','1'); }catch(e){}
    });
  }

  // Importante: escucha en toda la web, no solo en el chat.
  ...
  function playAlarm(kind){
    safe(function(){
      ...
      var gainValue = kind === 'human' ? 0.36 : 0.22;
      ...
      freqs.forEach(function(f,i){
        ...
        g.gain.exponentialRampToValueAtTime(gainValue,t+i*0.16+0.025);
  ...
  // 2) Notificación de primer usuario: escanea aunque el chat esté cerrado.
  var notified = {};
  try{ notified = JSON.parse(sessionStorage.getItem('tu_final5_notified') || '{}'); }catch(e){ notified = {}; }

  function saveNotified(){
    try{ sessionStorage.setItem('tu_final5_notified',JSON.stringify(notified)); }catch(e){}
  }
  ...
  // 3) Amarillo persistente: limpiar los campos que probablemente lo reactivan.
  function clearYellowData(id){
    if(!id) return updateChat(id,{});
  ...
  function clearYellowVisual(id){
    qa('.chat-tab,.chat-list-item,.chat-inbox-item,[data-chat-id]').forEach(function(el){
      ...
      // Sacar todas las clases que vimos usar para amarillo.
      [
        'new','unread','waiting','priority','prioridad',
        'tu-state-unread','has-new','hasNew','is-new',
        'esperando','pendiente','highlight','yellow'
      ].forEach(function(cls){ el.classList.remove(cls); });

      // Mantener rojo solo si realmente es humano no atendido.
      var c = id ? (chats()[id] || {}) : {};
      var keepRed = !!(c.humanRequested && !c.humanMode);
      if(!keepRed){
        el.style.borderColor = '';
        el.style.boxShadow = '';
        el.style.background = '';
      ...
      qa('*',el).forEach(function(n){
        ...
        if(/^(NUEVO|PRIORIDAD|ESPERANDO|PENDIENTE)$/i.test(txt)){
          n.textContent = '';
        }
      });

      var badge = q('.badge,.tu-state-badge',el);
      if(badge && /(NUEVO|PRIORIDAD|ESPERANDO|PENDIENTE)/i.test(badge.textContent || '')){
        badge.textContent = '';
      }
    });
  }
  ...
  var oldOpen = window.abrirChatAdmin;
  if(typeof oldOpen === 'function' && !oldOpen.__tuFinal5Open){
    var wrappedOpen = function(id){
      var c = id ? (chats()[id] || {}) : {};
      var wasHuman = !!(c.humanRequested || c.waitingHuman || c.priority || c.prioridad);

      if(id){
        // Abrir = leído. Si era humano: deja humanRequested, pero limpia amarillo.
        updateChat(id,{
          unreadAdmin:false,
          unread:false,
          hasNew:false,
      ...
      setTimeout(function(){ if(id){ clearYellowVisual(id); clearYellowData(id); } },200);
      setTimeout(function(){ if(id){ clearYellowVisual(id); clearYellowData(id); } },900);
      setTimeout(function(){ if(id){ clearYellowVisual(id); } },1600);

      return r;
    };
    wrappedOpen.__tuFinal5Open = 1;
    window.abrirChatAdmin = wrappedOpen;
    try{ abrirChatAdmin = wrappedOpen; }catch(e){}
  }

  var oldSend = window.enviarChatAdmin;
  if(typeof oldSend === 'function' && !oldSend.__tuFinal5Send){
    var wrappedSend = async function(){
      var id = adminId();
      var r = await oldSend.apply(this,arguments);
  ...
  // 4) Humano: texto nuevo, ring cada 10s, fallback 00.
  ...
  function watchHuman(){
    Object.entries(chats()).forEach(function(pair){
      ...
      if(!c.callUntil && !c.updatedAt) return;
  ...
  function checkHumanFallback(){
    Object.entries(chats()).forEach(function(pair){
      var id = pair[0];
      var c = pair[1] || {};
      if(!c.callUntil || !(c.humanRequested || c.waitingHuman || c.priority || c.prioridad)) return;
      if(fallbackSent[id] || c.humanFallbackSent) return;
  ...
  // Reescribe visualmente el texto viejo de humano al nuevo sin duplicar mensajes.
  function replaceHumanText(){
    qa('#chat-popover.open .chat-bubble.admin').forEach(function(b){
      var t = b.innerText || '';
      if(/Javier responderá personalmente|Para dejar tu consulta necesito|Para dejarle tu consulta necesito|Llamando a Javier/i.test(t)){
        b.innerHTML = humanText(0).replace(/\n/g,'<br>') + '<div class="chat-meta">Ahora</div>';
      }
    });
  }

  var oldResponder = window.responderAutomaticoChat;
  if(typeof oldResponder === 'function' && !oldResponder.__tuFinal5Responder){
    var wrappedResponder = async function(id,text){
      var c = chats()[id] || {};
      ...
      var r = await oldResponder.apply(this,arguments);
      setTimeout(watchHuman,300);
      return r;
    };
    wrappedResponder.__tuFinal5Responder = 1;
    window.responderAutomaticoChat = wrappedResponder;
    try{ responderAutomaticoChat = wrappedResponder; }catch(e){}
  }

  // 5) Javier online estable: sin intermitencia.
  function updateVisitorHeader(){
    if(isAdmin()) return;
    var id = visitorId();
    if(!id) return;
  ...
  // 6) Evitar que "mi nombre es Mariela" dispare catálogo/modelos.
  var oldBuscar = (typeof buscarRespuestaAsistente === 'function') ? buscarRespuestaAsistente : null;
  if(oldBuscar && !oldBuscar.__tuFinal5NameGuard){
    var wrappedBuscar = function(text){
      var x = norm(text);
      if(/^(mi nombre es|me llamo|soy)\s+[a-zñ\s]{2,30}$/.test(x)){
        return 'Gracias 😊 ¿En qué puedo ayudarte?';
      }
      return oldBuscar.apply(this,arguments);
    };
    wrappedBuscar.__tuFinal5NameGuard = 1;
    buscarRespuestaAsistente = wrappedBuscar;
    try{ window.buscarRespuestaAsistente = wrappedBuscar; }catch(e){}
  }

  function css(){
    if(q('#tu-final5-css')) return;
    var st = document.createElement('style');
    st.id = 'tu-final5-css';
    st.textContent = `
      .tu-v28d-sound-unlock,
  ...
  setInterval(function(){
    removeAlertButtons();
    // final6e: notificación vieja desactivada; solo cuenta mensajes nuevos desde esta sesión
    keepOpenClean();
    // final6e: ring viejo desactivado; solo maneja final6d/final6e
    // final6e: fallback viejo desactivado
    // final6e: header visitante estable, sin alternar textos
    // final6e: no reescribir burbujas humanas viejas
  },800);
})();
...
(function(){
  ...
  setInterval(function(){
    // final6g: updateCounterVisual(); desactivado
    // final6e: fallback viejo desactivado
    // final6g: updateVisitorHeaderStable(); desactivado
    // final6g: removeOldHumanAutoDuplicates(); desactivado
  },700);
})();
...
// ─────────────────────────────────────────────────────────────────────────────
// TOMAUNO v28b FINAL 6E — CORE ESTABLE
// Ajustes del bloque A sin tocar layout/admin completo.
// - sin alarmas acumuladas por historial
// - llamada humana reciente con contador
// - no repetir fallback cada vez que escribe
// - foco del input visitante luego de respuesta
// - botón enviar a la derecha en celular
// - no saltar arriba al cambiar AUTO/HUMANO
// ─────────────────────────────────────────────────────────────────────────────
(function(){
  'use strict';

  function safe(fn){ try{return fn();}catch(e){ try{console.warn('TU final6e:',e);}catch(_){} } }
  function q(s,r){ return (r||document).querySelector(s); }
  function qa(s,r){ return Array.from((r||document).querySelectorAll(s)); }
  function now(){ return Date.now(); }
  function norm(s){
  ...
  // Sonido solo para chats/mensajes nuevos desde que abrió esta sesión.
  function notifyNewMessagesOnly(){
    if(!isAdmin()) return;
    var dbs = chats();

    if(!firstSnapshotReady){
      Object.keys(dbs).forEach(function(id){ knownChats[id] = true; });
    ...
    Object.entries(dbs).forEach(function(pair){
      ...
      var key = id + '|' + String(last.createdAt || '') + '|' + String(last.text || '').slice(0,60);
      if(notified[key]) return;

      // No sonar por backlog/historial ni por sonidos pendientes.
  ...
  function startRing(id){
    ...
    function tick(){
      if(!ringTimers[id]) return;
      if(Date.now() - started > 60000){ stopRing(id); return; }
      sound('human');
      ringTimers[id] = setTimeout(tick,10000);
    }
    ringTimers[id] = true;
  ...
  // Wrapper final: evita repetir fallback y controla humano sin pasar al cerebro.
  var oldResponder = window.responderAutomaticoChat;
  if(typeof oldResponder === 'function' && !oldResponder.__tuFinal6e){
    var responder6e = async function(id,text){
      var c = chats()[id] || {};
      ...
      // Si ya venció/pidió WhatsApp y todavía no dejó teléfono: no repetir el mismo mensaje.
      if((c.humanFallbackSent || c.waitingHuman) && !hasPhone(text)){
        await updateChat(id,{
          unreadAdmin:true,
          updatedAt:Date.now(),
          lastUserMsg:text,
    ...
    responder6e.__tuFinal6e = 1;
    window.responderAutomaticoChat = responder6e;
    try{ responderAutomaticoChat = responder6e; }catch(e){}
  }

  function updateCounter(){
    var id = isAdmin() ? adminId() : visitorId();
    var c = chats()[id] || {};
    if(!c.humanRequested || !c.callUntil || c.humanFallbackSent) return;
    var left = Math.max(0,Math.ceil((Number(c.callUntil)-Date.now())/1000));

    qa('#chat-popover.open .chat-bubble.admin').forEach(function(b){
      var t = b.innerText || '';
      if(/Aguardá un momento|intentar contactar con Javier|para intentar conectar/i.test(t)){
        b.innerHTML = '<div class="tu6e-call">'+callText(left).replace(/\n/g,'<br>')+'</div><div class="chat-meta">Ahora</div>';
      }
    });
  }
  ...
  function fallbackOnce(){
    Object.entries(chats()).forEach(function(pair){
      var id = pair[0], c = pair[1] || {};
      if(!c.humanRequested || !c.callUntil || c.humanFallbackSent || fallbackSent[id]) return;
      if(Date.now() < Number(c.callUntil)) return;
  ...
  // AUTO/HUMANO no debe saltar al inicio.
  ...
  if(typeof oldToggle === 'function' && !oldToggle.__tu6e){
    ...
    toggle6e.__tu6e = 1;
    window.toggleModoAsistenteChat = toggle6e;
    try{ toggleModoAsistenteChat = toggle6e; }catch(e){}
  }

  // Foco cómodo luego de respuestas.
  var lastBubble = '';
  function focusInputAfterReply(){
    if(isAdmin()) return;
    ...
    var admins = qa('.chat-bubble.admin',box);
    if(!admins.length) return;
    var b = admins[admins.length-1];
    var key = (b.innerText || '').slice(0,120)+'|'+(b.innerText||'').length;
    if(key && key !== lastBubble){
      lastBubble = key;
      setTimeout(function(){
  ...
  function css(){
    ...
    st.textContent = `
      @media(max-width:700px){
        ...
        #chat-popover.open #chat-text{
          min-width:0!important;
          width:100%!important;
          font-size:17px!important;
        }
        #chat-popover.open .chat-send{
  ...
  setInterval(function(){
    // final6f: notificaciones por mensaje desactivadas; solo eventos nuevos/controlados
    // final6g: updateCounter(); desactivado
    // final6g: fallbackOnce(); desactivado
    focusInputAfterReply();
  },700);
})();
...
// ─────────────────────────────────────────────────────────────────────────────
// TOMAUNO v28b FINAL 6F — CORE FIX
// Ajustes mínimos sobre final6e.
// - scroll visitante libre en textos largos
// - sonido no se dispara por cada mensaje
// - llamada/fallback con burbuja blanca
// - después de registrar WhatsApp el asistente sigue respondiendo
// - minimizar no reabre al instante
// - keywords múltiples del cerebro separadas por coma
// - título visitante respeta AUTO/HUMANO
// ─────────────────────────────────────────────────────────────────────────────
(function(){
  'use strict';

  function safe(fn){ try{return fn();}catch(e){ try{console.warn('TU final6f:',e);}catch(_){} } }
  function q(s,r){ return (r||document).querySelector(s); }
  function qa(s,r){ return Array.from((r||document).querySelectorAll(s)); }
  function now(){ return Date.now(); }
  function norm(s){
  ...
  // Solo sonido para chat nuevo creado en esta sesión, no por cada mensaje.
  function notifyNewChatOnly(){
    if(!isAdmin()) return;
    var dbs = chats();

    if(!initialDone){
      Object.keys(dbs).forEach(function(id){ known[id] = true; });
      initialDone = true;
      return;
    }
    ...
    Object.entries(dbs).forEach(function(pair){
      var id = pair[0], c = pair[1] || {};
      if(known[id]) return;
      var msg = latestUser(c);
      if(!msg) return;
      if(Number(msg.createdAt||0) < bootAt - 1500) return;
      known[id] = true;
      playSound('normal');
    });
  }

  // Scroll visitante: si el usuario toca/rueda, no bajar automáticamente.
  function lockScroll(){
    userScrollLockUntil = Date.now() + 15000;
  }
  ...
  // Minimizar: no reabrir al instante.
  var oldClose = window.cerrarChatPopover;
  if(typeof oldClose === 'function' && !oldClose.__tu6f){
    var close6f = function(){
      if(!isAdmin()){
        visitorClosedUntil = Date.now() + 10000;
        try{ sessionStorage.setItem('tu_visitor_closed_until',String(visitorClosedUntil)); }catch(e){}
      }
      return oldClose.apply(this,arguments);
    };
    close6f.__tu6f = 1;
    window.cerrarChatPopover = close6f;
    try{ cerrarChatPopover = close6f; }catch(e){}
  }

  var oldOpenVisitor = window.abrirChatVisitante;
  if(typeof oldOpenVisitor === 'function' && !oldOpenVisitor.__tu6f){
    var openVisitor6f = function(id,silent){
      var until = visitorClosedUntil;
      try{ until = Math.max(until,Number(sessionStorage.getItem('tu_visitor_closed_until')||0)); }catch(e){}
      if(!isAdmin() && silent && Date.now() < until) return;
      return oldOpenVisitor.apply(this,arguments);
    };
    openVisitor6f.__tu6f = 1;
    window.abrirChatVisitante = openVisitor6f;
    try{ abrirChatVisitante = openVisitor6f; }catch(e){}
  }
  ...
  function isHumanRequest(text){
    ...
    return [
      ...
      'puedes llamar a javier','podes llamar a javier','podrias llamar a javier','podrías llamar a javier',
      'llama a javier','llamar a javier','atencion humana','atención humana','humano',
      'cel de javier','celular de javier','telefono de javier','teléfono de javier','whatsapp de javier'
    ].map(norm).some(function(p){ return x.includes(p); });
  }
  ...
  function startRing(id){
    if(!id || humanRingTimers[id]) return;
    var started = Date.now();
    function tick(){
      if(!humanRingTimers[id]) return;
      if(Date.now() - started > 60000){
        stopRing(id);
        return;
      }
      playSound('human');
      humanRingTimers[id] = setTimeout(tick,10000);
    }
    humanRingTimers[id] = true;
    tick();
  }

  function stopRing(id){
    if(humanRingTimers[id] && humanRingTimers[id] !== true) clearTimeout(humanRingTimers[id]);
    delete humanRingTimers[id];
  }
  ...
  var oldResponder = window.responderAutomaticoChat;
  if(typeof oldResponder === 'function' && !oldResponder.__tu6f){
    var responder6f = async function(id,text){
      var c = chats()[id] || {};
      ...
      if(isHumanRequest(text)){
        var key = id + '|' + norm(text);
        if(handledHuman[key]) return;
        handledHuman[key] = true;
    ...
    responder6f.__tu6f = 1;
    window.responderAutomaticoChat = responder6f;
    try{ responderAutomaticoChat = responder6f; }catch(e){}
  }

  function fallbackOnce(){
    Object.entries(chats()).forEach(function(pair){
      var id = pair[0], c = pair[1] || {};
      if(!c.humanRequested || !c.callUntil || c.humanFallbackSent || fallbackDone[id]) return;
      if(Date.now() < Number(c.callUntil)) return;
  ...
  function updateCounter(){
    ...
    qa('#chat-popover.open .chat-bubble.admin').forEach(function(b){
      var t = b.innerText || '';
      if(/Aguardá un momento|intentar conectar con Javier|intentar contactar con Javier/i.test(t)){
        b.innerHTML = '<div class="tu6f-white">'+callText(left).replace(/\n/g,'<br>')+'</div><div class="chat-meta">Ahora</div>';
      }
    });
  }

  function styleFallbackWhite(){
    qa('#chat-popover.open .chat-bubble.admin').forEach(function(b){
      var t = b.innerText || '';
      if(/Al parecer Javier está ocupado|Ya quedó registrada tu consulta/i.test(t) && !b.querySelector('.tu6f-white')){
        b.innerHTML = '<div class="tu6f-white">'+t.replace(/\n/g,'<br>')+'</div>';
      }
    });
  }

  // Keywords múltiples separadas por coma para /comandos.
  ...
  var oldBuscar = (typeof buscarRespuestaAsistente === 'function') ? buscarRespuestaAsistente : null;
  if(oldBuscar && !oldBuscar.__tu6fKeys){
    var buscar6f = function(text){
      var x = String(text||'').trim();
      if(x.startsWith('/')){
        try{
          if(typeof asistenteKnowledgeEntries === 'function'){
            var entries = asistenteKnowledgeEntries();
            var cmd = x.toLowerCase();
            for(var i=0;i<entries.length;i++){
              var k = entries[i][1] || {};
              var keys = splitKeys([k.command||'',k.keys||'',k.titulo||''].join(','));
              for(var j=0;j<keys.length;j++){
                if(keys[j].toLowerCase() === cmd && k.respuesta) return k.respuesta;
              }
            }
          }
        }catch(e){}
    ...
    buscar6f.__tu6fKeys = 1;
    buscarRespuestaAsistente = buscar6f;
    try{ window.buscarRespuestaAsistente = buscar6f; }catch(e){}
  }

  // Header visitante estable según modo.
  function updateVisitorHeader(){
    if(isAdmin()) return;
    var id = visitorId();
    var c = id ? (chats()[id] || {}) : {};
    ...
    var onlineHuman = !!(c.javierOnline && c.javierOnlineAt && Date.now()-Number(c.javierOnlineAt)<15*60*1000 && !isAuto());

    if(onlineHuman){
      title.textContent = 'JAVIER ONLINE';
      if(sub) sub.innerHTML = '🟢 Javier está en línea';
    }else{
      title.textContent = 'CHAT TOMAUNO';
      if(sub) sub.textContent = 'Asistente Tomauno';
    }
  }

  var oldToggle = window.toggleModoAsistenteChat;
  if(typeof oldToggle === 'function' && !oldToggle.__tu6fToggle){
    var toggle6f = function(){
      var box = q('#chat-popover.open .chat-msgs');
      var oldTop = box ? box.scrollTop : null;
      var r = oldToggle.apply(this,arguments);
      setTimeout(function(){
        if(isAuto()){
          var id = adminId();
          if(id) updateChat(id,{javierOnline:false,javierOnlineAt:0,humanMode:false});
        }
        var b = q('#chat-popover.open .chat-msgs');
        if(b && oldTop !== null) b.scrollTop = oldTop;
      },150);
      return r;
    };
    toggle6f.__tu6fToggle = 1;
    window.toggleModoAsistenteChat = toggle6f;
    try{ toggleModoAsistenteChat = toggle6f; }catch(e){}
  }

  function css(){
    if(q('#tu6f-css')) return;
    var st = document.createElement('style');
    st.id = 'tu6f-css';
    st.textContent = `
      .tu6f-white{
  ...
  setInterval(function(){
    // final6g: notifyNewChatOnly(); desactivado
    // final6g: updateCounter(); desactivado
    // final6g: fallbackOnce(); desactivado
    // final6g: styleFallbackWhite(); desactivado
    // final6g: updateVisitorHeader(); desactivado
  },700);
})();
...
// ─────────────────────────────────────────────────────────────────────────────
// TOMAUNO v28b FINAL 6G — ESTABILIDAD SCROLL / NOTIF / HUMANO / CEREBRO
// Cambios controlados:
// - scroll visitante libre al leer
// - notificaciones solo chat nuevo o llamada
// - humano solo 2 mensajes: llamada + fallback blanco
// - AUTO/HUMANO estable
// - título visitante: ASISTENTE TOMAUNO en auto / JAVIER ONLINE en manual
// - primeras reglas de cerebro: pagos, edad, dueño, cursos, 15 años
// ─────────────────────────────────────────────────────────────────────────────
(function(){
  'use strict';

  function safe(fn){ try{return fn();}catch(e){ try{console.warn('TU final6g:',e);}catch(_){} } }
  function q(s,r){ return (r||document).querySelector(s); }
  function qa(s,r){ return Array.from((r||document).querySelectorAll(s)); }
  function norm(s){
    return String(s||'').toLowerCase()
  ...
  // 1) Scroll visitante: al tocar o intentar subir, no forzar abajo por 20s.
  function lockVisitorScroll(){
    if(!q('#chat-popover.open #chat-text')) return;
    window.__tomaunoVisitorReadingUntil = Date.now() + 20000;
  }
  ['wheel','touchstart','touchmove','pointerdown','scroll'].forEach(function(ev){
    document.addEventListener(ev,function(e){
      if(e.target && e.target.closest && e.target.closest('#chat-popover.open .chat-msgs')){
        lockVisitorScroll();
      }
  ...
  // 2) Sonidos: solo nuevo chat / llamada.
  function play(kind){
    safe(function(){
      var AC = window.AudioContext || window.webkitAudioContext;
      if(!AC) return;
      ...
      freqs.forEach(function(f,i){
        ...
        o.connect(g); g.connect(ctx.destination);
        o.start(t+i*.14); o.stop(t+i*.14+.12);
      });
      setTimeout(function(){ if(ctx.close) ctx.close(); },900);
    });
  }
  ...
  function notifyNewChatOnly(){
    ...
    Object.entries(dbs).forEach(function(pair){
      var id=pair[0], c=pair[1]||{};
      if(knownChats[id]) return;
      var first=firstUserMessage(c);
      if(!first) return;
      if(Number(first.createdAt||0) < bootAt - 1500) return;
      knownChats[id]=true;
      play('normal');
    });
  }

  // 3) Humano: solo mensaje llamada + fallback blanco.
  function isHumanRequest(text){
    var x=norm(text);
    return [
      'pasame con javier','quiero hablar con javier','quiero hablar con el dueno','quiero hablar con el dueño',
      'pasame con el dueno','pasame con el dueño','quiero consultar a javier','javier se encuentra',
      'me puedo comunicar con javier','puedo hablar con el fotografo','puedo hablar con el fotógrafo',
      'me pasas con el fotografo','me pasas con el fotógrafo','llamar a javier','llama a javier',
      'podes llamar a javier','puedes llamar a javier','humano','atencion humana','atención humana',
      'cel de javier','celular de javier','telefono de javier','teléfono de javier','whatsapp de javier'
    ].map(norm).some(function(p){ return x.includes(p); });
  }
  function hasPhone(text){ return String(text||'').replace(/[^\d+]/g,'').length>=8; }
  function callText(sec){
    sec=Math.max(0,Number(sec||0));
    return '📞 Aguardá un momento por favor, voy a intentar contactar con Javier para que te responda por acá 😊\n\n⏱️ '+String(sec).padStart(2,'0')+'s para intentar conectar con Javier';
  }
  ...
  var prevResponder = window.responderAutomaticoChat;
  if(typeof prevResponder === 'function' && !prevResponder.__tu6g){
    var responder6g = async function(id,text){
      var c=chats()[id]||{};

      if((c.humanRequested || c.waitingHuman || c.humanFallbackSent) && hasPhone(text)){
        stopRing(id);
        await updateChat(id,{humanRequested:false,waitingHuman:false,humanFallbackSent:false,priority:false,prioridad:false,unreadAdmin:true,updatedAt:Date.now(),lastUserMsg:text,lastUserAt:Date.now()});
      ...
      if(isHumanRequest(text)){
        ...
        handledHuman[key]=true;
        var t=Date.now();
        await updateChat(id,{humanRequested:true,waitingHuman:false,humanFallbackSent:false,priority:false,prioridad:false,unreadAdmin:true,callUntil:t+60000,updatedAt:t,lastUserMsg:text,lastUserAt:t});
        await pushMessage(id,{from:'admin',auto:true,text:callText(60),time:chatTimeSafe(),createdAt:Date.now(),systemCall:true,callUntil:t+60000});
        if(isAdmin()) startRing(id);
        return;
      }

      return prevResponder.apply(this,arguments);
    };
    responder6g.__tu6g = 1;
    window.responderAutomaticoChat = responder6g;
    try{ responderAutomaticoChat = responder6g; }catch(e){}
  }
  ...
  function updateCounter(){
    ...
    qa('#chat-popover.open .chat-bubble.admin').forEach(function(b){
      var t=b.innerText||'';
      if(/Aguardá un momento|intentar conectar con Javier|intentar contactar con Javier/i.test(t)){
        b.innerHTML='<div class="tu6g-white">'+callText(left).replace(/\n/g,'<br>')+'</div><div class="chat-meta">Ahora</div>';
      }
    });
  }
  ...
  function fallbackOnce(){
    Object.entries(chats()).forEach(function(pair){
      ...
      if(!c.humanRequested || !c.callUntil || c.humanFallbackSent || fallbackSent[id]) return;
      if(Date.now()<Number(c.callUntil)) return;
      fallbackSent[id]=true; stopRing(id);
  ...
  function styleWhite(){
    qa('#chat-popover.open .chat-bubble.admin').forEach(function(b){
      var t=b.innerText||'';
      if(/Al parecer Javier está ocupado|Ya quedó registrada tu consulta/i.test(t) && !b.querySelector('.tu6g-white')){
        b.innerHTML='<div class="tu6g-white">'+t.replace(/\n/g,'<br>')+'</div>';
      }
      if(/Javier no pudo responder en este momento/i.test(t)){
        b.remove();
      }
    });
  }

  // 4) Cerebro: reglas puntuales antes de cursos.
  function splitKeys(raw){
    return String(raw||'').split(/[,\n;]/).map(function(x){return x.trim();}).filter(Boolean);
  }
  function findKnowledgeByTerms(terms){
    try{
      if(typeof asistenteKnowledgeEntries !== 'function') return '';
      var entries=asistenteKnowledgeEntries();
      for(var i=0;i<entries.length;i++){
        var k=entries[i][1]||{};
        var hay=norm([k.titulo||'',k.command||'',k.keys||'',k.respuesta||''].join(' '));
        if(terms.some(function(t){return hay.includes(norm(t));}) && k.respuesta) return k.respuesta;
      }
    }catch(e){}
    return '';
  }
  ...
  var oldBuscar = (typeof buscarRespuestaAsistente === 'function') ? buscarRespuestaAsistente : null;
  if(oldBuscar && !oldBuscar.__tu6gBrain){
    var buscar6g = function(text){
      var x=norm(text);
      if(/forma.*pago|formas.*pago|medios.*pago|como.*pago|se puede pagar|transferencia|efectivo|mercado pago|tarjeta/.test(x)){
        return findKnowledgeByTerms(['formas de pago','medios de pago','pago','pagos','transferencia','efectivo','mercado pago']) || 'Podés consultar las formas de pago disponibles directamente por WhatsApp o con Javier para confirmar la opción más cómoda.';
      }
      if(/edad|años|anos|a partir de que edad|a partir de cuantos|puedo anotarme|puedo inscribirme/.test(x)){
        return findKnowledgeByTerms(['edad','edades','años','inscripción','inscripcion','anotarme']) || 'Depende de la actividad o curso. Decime cuál te interesa y te confirmo la edad recomendada o mínima.';
      }
      if(/dueño|dueno|propietario|nombre del dueño|quien es javier|quien es el fotografo|quién es el fotógrafo/.test(x)){
        return findKnowledgeByTerms(['dueño','dueno','propietario','javier','fotógrafo','fotografo']) || 'El estudio es de Javier Móttola, fotógrafo y director de Tomauno.';
      }
      if(/que cursos dan|qué cursos dan|cursos dan|que cursos tienen|qué cursos tienen|hay cursos|cursos activos|dictan cursos/.test(x)){
        return findKnowledgeByTerms(['cursos activos','curso','cursos','capacitaciones']) || (typeof respuestaCursosActivosChat === 'function' ? respuestaCursosActivosChat() : 'Puedo mostrarte los cursos activos. Tocá el botón Cursos o escribí “cursos activos”.');
      }
      if(/15 años|quince|quinceañera|15 de mi hija|presupuesto.*15|fotos.*15/.test(x)){
        return findKnowledgeByTerms(['15 años','quince','quinceañera','pack15','sesión 15','sesion 15']) || 'Para fotos de 15 años tenemos distintas opciones de cobertura/sesión. Si querés, te paso las opciones disponibles o podés dejar tu WhatsApp para enviarte el detalle.';
      }

      // Comandos con varias keys separadas por coma.
      if(String(text||'').trim().startsWith('/')){
        try{
          if(typeof asistenteKnowledgeEntries === 'function'){
            var cmd=String(text||'').trim().toLowerCase();
            var entries=asistenteKnowledgeEntries();
            for(var i=0;i<entries.length;i++){
              var k=entries[i][1]||{};
              var keys=splitKeys([k.command||'',k.keys||'',k.titulo||''].join(','));
    ...
    buscar6g.__tu6gBrain = 1;
    buscarRespuestaAsistente = buscar6g;
    try{ window.buscarRespuestaAsistente = buscar6g; }catch(e){}
  }
  ...
  function updateHeader(){
    ...
    var id=visitorId();
    var c=id?(chats()[id]||{}):{};
    var humanOnline=!!(c.javierOnline && c.javierOnlineAt && Date.now()-Number(c.javierOnlineAt)<15*60*1000 && !isAuto());
    if(humanOnline){
      title.textContent='JAVIER ONLINE';
      if(sub) sub.innerHTML='🟢 Javier está en línea';
    }else{
  ...
  function css(){
    ...
    st.textContent=`
      ...
      .tu6g-white::first-line{color:#e8000a!important;font-weight:900!important;}
...
// ─────────────────────────────────────────────────────────────────────────────
// TOMAUNO v28b FINAL 6H — CORE REAL
// Cambios finales directos:
// - scroll visitante realmente libre al leer
// - sonidos solo chat nuevo o llamada
// - AUTO/HUMANO estable por chat
// - cerebro por prioridad usando keys/título/command, no respuesta completa
// - fallback humano único en blanco
// ─────────────────────────────────────────────────────────────────────────────
(function(){
  'use strict';

  function safe(fn){ try{return fn();}catch(e){ try{console.warn('TU final6h:',e);}catch(_){} } }
  function q(s,r){ return (r||document).querySelector(s); }
  function qa(s,r){ return Array.from((r||document).querySelectorAll(s)); }
  function norm(s){
    return String(s||'').toLowerCase()
  ...
  function adminId(){ return safe(function(){ return window.currentOpenChatId || currentOpenChatId || ''; }) || ''; }
  function visitorId(){ return safe(function(){ return window.currentVisitorChatId || currentVisitorChatId || sessionStorage.getItem('tomauno-chat-id') || ''; }) || ''; }
  function isAuto(){ return safe(function(){ return asistenteModo() === 'automatico'; }) !== false; }
  ...
  function lockVisitorScroll(){
    if(!q('#chat-popover.open #chat-text')) return;
    window.__tomaunoVisitorReadingUntil = Date.now() + 25000;
  }

  ['wheel','touchstart','touchmove','pointerdown','scroll','mousedown'].forEach(function(ev){
    document.addEventListener(ev,function(e){
      if(e.target && e.target.closest && e.target.closest('#chat-popover.open .chat-msgs')){
        lockVisitorScroll();
      }
  ...
  window.tomaunoCanAutoScrollVisitor = function(box){
    if(!box) return true;
    if(!q('#chat-popover.open #chat-text') || q('#chat-popover.open #chat-admin-text')) return true;
    const distance = box.scrollHeight - box.clientHeight - box.scrollTop;
    if(window.__tomaunoVisitorReadingUntil && Date.now() < window.__tomaunoVisitorReadingUntil) return false;
    if(distance > 90) return false;
    return true;
  };

  // Refuerzo por si alguna capa llama scrollIntoView.
  var oldScrollInto = Element.prototype.scrollIntoView;
  if(oldScrollInto && !oldScrollInto.__tu6h){
    Element.prototype.scrollIntoView = function(){
      if(this.closest && this.closest('#chat-popover.open .chat-msgs')){
        var box = this.closest('.chat-msgs');
        if(window.tomaunoCanAutoScrollVisitor && !window.tomaunoCanAutoScrollVisitor(box)) return;
      }
      return oldScrollInto.apply(this,arguments);
    };
    Element.prototype.scrollIntoView.__tu6h = 1;
  }

  // 2) Sonidos controlados.
  function play(kind){
    safe(function(){
      var AC = window.AudioContext || window.webkitAudioContext;
      if(!AC) return;
  ...
  function notifyOnlyNewChat(){
    ...
    Object.entries(dbs).forEach(function(pair){
      ...
      if(knownChats[id]) return;
      var first = firstUserMessage(c);
      if(!first || Number(first.createdAt||0) < bootAt - 1500) return;
      knownChats[id] = true;
      play('normal');
    });
  }

  // 3) AUTO/HUMANO estable por chat.
  window.toggleModoAsistenteChat = async function(){
    var id = adminId();
    if(id && q('#chat-popover.open #chat-admin-text')){
      var c = chats()[id] || {};
      var toHuman = !(c.humanMode || Number(c.manualUntil||0) > Date.now());
      if(toHuman){
        await updateChat(id,{
          humanMode:true,
          manualUntil:Date.now()+3600000,
        ...
        safe(function(){ toast('👤 Este chat queda en humano', true); });
      }else{
        await updateChat(id,{
          humanMode:false,
          manualUntil:0,
        ...
        safe(function(){ toast('🤖 Este chat vuelve a automático', true); });
      }
      setTimeout(function(){ safe(function(){ abrirChatAdmin(id,true); }); },80);
      return;
    }

    // Si no hay chat activo, toggle global.
    var nuevo = isAuto() ? 'manual' : 'automatico';
    if(typeof db !== 'undefined' && typeof ref !== 'undefined' && typeof update !== 'undefined'){
      await update(ref(db,'tomauno/asistente'),{modo:nuevo}).catch(function(){});
    }
    safe(function(){ asistenteDB = Object.assign({}, asistenteDB || {}, {modo:nuevo}); });
    safe(function(){ toast(nuevo === 'automatico' ? '🤖 Asistente automático ON' : '👤 Asistente manual', true); });
  };
  ...
  var prevResponder = window.responderAutomaticoChat;
  if(typeof prevResponder === 'function' && !prevResponder.__tu6h){
    var responder6h = async function(id,text){
      var c = chats()[id] || {};
      if(c.humanMode || Number(c.manualUntil||0) > Date.now()) return;
    ...
    responder6h.__tu6h = 1;
    window.responderAutomaticoChat = responder6h;
    try{ responderAutomaticoChat = responder6h; }catch(e){}
  }
  ...
  function updateCounter(){
    ...
    qa('#chat-popover.open .chat-bubble.admin').forEach(function(b){
      var t=b.innerText||'';
      if(/Aguardá un momento|intentar conectar con Javier|intentar contactar con Javier/i.test(t)){
        b.innerHTML='<div class="tu6h-white">'+callText(left).replace(/\n/g,'<br>')+'</div><div class="chat-meta">Ahora</div>';
      }
    });
  }
  ...
  function styleWhite(){
    qa('#chat-popover.open .chat-bubble.admin').forEach(function(b){
      var t=b.innerText||'';
      if(/Al parecer Javier está ocupado|Ya quedó registrada tu consulta/i.test(t) && !b.querySelector('.tu6h-white')){
        b.innerHTML='<div class="tu6h-white">'+t.replace(/\n/g,'<br>')+'</div>';
      }
      if(/Javier no pudo responder en este momento/i.test(t)) b.remove();
    });
  }

  // 5) Cerebro con prioridad por META, no por respuesta completa.
  function splitKeys(raw){
    return String(raw||'').split(/[,\n;]/).map(function(x){return x.trim();}).filter(Boolean);
  }

  function findByMeta(terms){
    try{
      if(typeof asistenteKnowledgeEntries !== 'function') return '';
      var entries = asistenteKnowledgeEntries();
      var wanted = terms.map(norm);
      for(var i=0;i<entries.length;i++){
        var k = entries[i][1] || {};
        var meta = norm([k.titulo||'',k.command||'',k.keys||''].join(' '));
        if(wanted.some(function(t){ return t && meta.includes(t); }) && k.respuesta) return k.respuesta;
      }
    }catch(e){}
    return '';
  }
  ...
  var oldBuscar = (typeof buscarRespuestaAsistente === 'function') ? buscarRespuestaAsistente : null;
  if(oldBuscar && !oldBuscar.__tu6hBrain){
    var buscar6h = function(text){
      var x = norm(text);
      if(/forma.*pago|formas.*pago|medio.*pago|medios.*pago|como.*pago|se puede pagar|transferencia|efectivo|mercado pago|tarjeta/.test(x)){
        return findByMeta(['formas de pago','medios de pago','pago','pagos','transferencia','efectivo','mercado pago']) || 'Las formas de pago pueden variar según la actividad. Escribinos por WhatsApp y te confirmamos las opciones disponibles.';
      }
      if(/edad|a partir de cuantos|a partir de que edad|puedo anotarme|puedo inscribirme|cuantos años|cuántos años/.test(x)){
        return findByMeta(['edad','edades','años','inscripción','inscripcion','anotarme']) || 'Depende de la actividad o curso. Decime cuál te interesa y te confirmo la edad recomendada o mínima.';
      }
      if(/dueño|dueno|propietario|nombre del dueño|javier|mottola|fotografo|fotógrafo/.test(x) && /nombre|dueño|dueno|propietario|quien|quién|fotografo|fotógrafo/.test(x)){
        return findByMeta(['dueño','dueno','propietario','javier','mottola','fotógrafo','fotografo']) || 'El estudio es de Javier Móttola, fotógrafo y director de Tomauno.';
      }
      if(/que cursos dan|qué cursos dan|cursos dan|que cursos tienen|qué cursos tienen|hay cursos|cursos activos|dictan cursos|curso disponibles|cursos disponibles/.test(x)){
        return findByMeta(['cursos activos','cursos','capacitaciones']) || (typeof listaCursosAI === 'function' ? listaCursosAI('') : 'Puedo mostrarte los cursos activos. Tocá el botón Cursos o escribí “cursos activos”.');
      }
      if(/15 años|quince|quinceañera|quinceanera|15 de mi hija|presupuesto.*15|fotos.*15|fiesta.*15/.test(x)){
        return findByMeta(['15 años','quince','quinceañera','quinceanera','pack15','sesión 15','sesion 15']) || 'Para fotos de 15 años tenemos distintas opciones. Si querés, te paso las opciones disponibles o podés dejar tu WhatsApp para enviarte el detalle.';
      }
      if(String(text||'').trim().startsWith('/')){
        try{
          if(typeof asistenteKnowledgeEntries === 'function'){
    ...
    buscar6h.__tu6hBrain = 1;
    buscarRespuestaAsistente = buscar6h;
    try{ window.buscarRespuestaAsistente = buscar6h; }catch(e){}
  }
  ...
  function css(){
    ...
    st.textContent=`
      ...
      .tu6h-white::first-line{color:#e8000a!important;font-weight:900!important;}
      #chat-popover.open .chat-msgs{scroll-behavior:auto!important;overscroll-behavior:contain!important;}
    `;
    document.head.appendChild(st);
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// TOMAUNO v28b FINAL 6I — SCROLL / SONIDO / MODO
// Control final de:
// - scroll visitante al leer
// - silencio para "Nuevo mensaje web"
// - alarma Javier por watcher admin
// - foco input visitante
// - HUMANO/AUTO por chat
// - bandeja sin rebote
// ─────────────────────────────────────────────────────────────────────────────
(function(){
  'use strict';

  function safe(fn){ try{return fn();}catch(e){ try{console.warn('TU final6i:',e);}catch(_){} } }
  function q(s,r){ return (r||document).querySelector(s); }
  function qa(s,r){ return Array.from((r||document).querySelectorAll(s)); }
  ...
  // 1) SCROLL VISITANTE: bloqueo fuerte de scrollTop cuando el usuario está leyendo.
  function isVisitorMsgs(el){
    return !!(el && el.classList && el.classList.contains('chat-msgs') && q('#chat-popover.open #chat-text') && !q('#chat-popover.open #chat-admin-text'));
  }

  function lockVisitorReading(){
    if(!q('#chat-popover.open #chat-text') || q('#chat-popover.open #chat-admin-text')) return;
    window.__tomaunoVisitorReadingUntil = Date.now() + 30000;
  }
  ...
  safe(function(){
    ...
    var desc = Object.getOwnPropertyDescriptor(proto,'scrollTop') || Object.getOwnPropertyDescriptor(Element.prototype,'scrollTop');
    if(!desc || !desc.set || desc.set.__tu6i) return;
    Object.defineProperty(proto,'scrollTop',{
      get:function(){ return desc.get.call(this); },
      set:function(v){
        if(isVisitorMsgs(this)){
          var distance = this.scrollHeight - this.clientHeight - desc.get.call(this);
          var locked = window.__tomaunoVisitorReadingUntil && Date.now() < window.__tomaunoVisitorReadingUntil;
          var wantsBottom = Number(v) >= this.scrollHeight - this.clientHeight - 5;
          if((locked || distance > 120) && wantsBottom){
            return;
          }
        }
    ...
    Object.getOwnPropertyDescriptor(proto,'scrollTop').set.__tu6i = 1;
  });

  var oldScrollInto = Element.prototype.scrollIntoView;
  if(oldScrollInto && !oldScrollInto.__tu6i){
    Element.prototype.scrollIntoView = function(){
      if(this.closest && this.closest('#chat-popover.open .chat-msgs')){
        var box = this.closest('.chat-msgs');
        if(isVisitorMsgs(box) && window.__tomaunoVisitorReadingUntil && Date.now() < window.__tomaunoVisitorReadingUntil) return;
      }
      return oldScrollInto.apply(this,arguments);
    };
    Element.prototype.scrollIntoView.__tu6i = 1;
  }

  // 2) SONIDO: bloquear definitivamente Nuevo mensaje web; permitir solo chat nuevo y llamada.
  var prevNotify = window.notifyAdminChat || (typeof notifyAdminChat !== 'undefined' ? notifyAdminChat : null);
  window.notifyAdminChat = function(title,body,chatId){
    var t = String(title||'');
    var b = String(body||'');
    if(/Nuevo mensaje web/i.test(t)) return;
    if(/mensaje/i.test(t) && !/chat nuevo|nuevo chat|llamada|javier|humana/i.test(t+b)) return;
  ...
  function startHumanRing(id){
    ...
    function tick(){
      if(!humanRingTimers[id]) return;
      if(Date.now() - started > 60000){ stopHumanRing(id); return; }
      alarm('human');
      humanRingTimers[id] = setTimeout(tick,10000);
    }
    humanRingTimers[id] = true;
  ...
  function watchHumanRequests(){
    ...
    Object.entries(chats()).forEach(function(pair){
      var id = pair[0], c = pair[1] || {};
      if(!(c.humanRequested && c.callUntil && !c.humanFallbackSent)) return;
      if(Number(c.callUntil) < Date.now()) return;
      if(humanRingSeen[id]) return;
      humanRingSeen[id] = true;
      startHumanRing(id);
    });
  }

  // 3) FOCO input visitante luego de respuesta / saludo.
  function focusVisitorInput(){
    ...
    if(key !== lastVisitorFocusKey){
      lastVisitorFocusKey = key;
      setTimeout(function(){ try{ inp.focus({preventScroll:true}); inp.setSelectionRange(inp.value.length,inp.value.length); }catch(e){} },180);
    }
  }

  // 4) MODO HUMANO/AUTO: controlar las dos funciones posibles.
  async function setChatMode(chatId,toHuman){
    if(!chatId) return;
    if(toHuman){
    ...
    setTimeout(function(){ safe(function(){ abrirChatAdmin(chatId,true); }); },80);
  }

  window.tomaunoToggleModoChatActual = async function(id){
    var chatId = id || adminId();
    if(!chatId) return window.toggleModoAsistenteChat && window.toggleModoAsistenteChat();
    var c = chats()[chatId] || {};
    var currentlyHuman = !!(c.humanMode || Number(c.manualUntil||0) > Date.now());
    return setChatMode(chatId,!currentlyHuman);
  };

  window.toggleModoAsistenteChat = async function(){
    var chatId = adminId();
    if(chatId && q('#chat-popover.open #chat-admin-text')){
      return window.tomaunoToggleModoChatActual(chatId);
    }
    var nuevo = isAuto() ? 'manual' : 'automatico';
    if(typeof db !== 'undefined' && typeof ref !== 'undefined' && typeof update !== 'undefined'){
      await update(ref(db,'tomauno/asistente'),{modo:nuevo}).catch(function(){});
  ...
  // 5) BANDEJA sin rebote.
  var oldPanel = window.abrirPanelChatsAdmin;
  if(typeof oldPanel === 'function' && !oldPanel.__tu6i){
    window.abrirPanelChatsAdmin = function(){
      panelLockUntil = Date.now() + 1200;
      safe(function(){ currentOpenChatId=''; window.currentOpenChatId=''; });
      return oldPanel.apply(this,arguments);
    };
    window.abrirPanelChatsAdmin.__tu6i = 1;
    try{ abrirPanelChatsAdmin = window.abrirPanelChatsAdmin; }catch(e){}
  }

  var oldOpenAdmin = window.abrirChatAdmin;
  if(typeof oldOpenAdmin === 'function' && !oldOpenAdmin.__tu6i){
    window.abrirChatAdmin = function(id,silent){
      if(Date.now() < panelLockUntil && silent) return;
      return oldOpenAdmin.apply(this,arguments);
    };
    window.abrirChatAdmin.__tu6i = 1;
  ...
  // 7) Marcar usuarios que dejaron WhatsApp para Javier.
  function markJavierLeads(){
    qa('.chat-tab,.chat-list-item,.chat-inbox-item,[data-chat-id]').forEach(function(el){
      var id = el.getAttribute('data-chat-id') || el.dataset.chatId || '';
      if(!id) return;
      var c = chats()[id] || {};
      var mark = !!(c.humanFallbackSent || c.humanConfirm || c.humanRequested);
      el.classList.toggle('javier-lead', mark);
      if(mark && !el.querySelector('.tu-lead-star')){
        el.insertAdjacentHTML('afterbegin','<span class="tu-lead-star" title="Consulta para Javier">⭐</span>');
      }
    });
  }

  function css(){
    if(q('#tu6i-css')) return;
    var st=document.createElement('style');
    st.id='tu6i-css';
    st.textContent=`
      .tu-lead-star{
        ...
        filter:drop-shadow(0 0 5px rgba(255,213,74,.55))!important;
      }
      .javier-lead{
        border-color:rgba(255,213,74,.65)!important;
      }
    `;
    document.head.appendChild(st);
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// TOMAUNO v28b FINAL 6L — SCROLL + JAVIER + ESTADOS
// - scroll visitante al inicio de la última respuesta
// - unifica TODAS las frases de Javier al mismo flujo con contador
// - alarma Javier por cada solicitud reciente
// - header único AUTO/HUMANO
// - color prioridad: rojo > amarillo > verde > gris + estrella
// - evita duplicar botón Ver cursos/servicios/eventos
// ─────────────────────────────────────────────────────────────────────────────
(function(){
'use strict';

function safe(fn){try{return fn()}catch(e){try{console.warn('TU final6l:',e)}catch(_){}}}
function q(s,r){return (r||document).querySelector(s)}
function qa(s,r){return Array.from((r||document).querySelectorAll(s))}
function norm(s){return String(s||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9ñ\s]/g,' ').replace(/\s+/g,' ').trim()}
...
// 1) SCROLL VISITANTE: respuesta nueva = inicio de la respuesta, salvo que el usuario esté leyendo.
function userTouchedScroll(){
  if(!isVisitor())return;
  visitorReadingUntil=Date.now()+30000;
  window.__tomaunoVisitorReadingUntil=visitorReadingUntil;
}
['wheel','touchstart','touchmove','pointerdown','mousedown','scroll'].forEach(ev=>{
  document.addEventListener(ev,e=>{
    if(e.target&&e.target.closest&&e.target.closest('#chat-popover.open .chat-msgs')) userTouchedScroll();
  },true);
});
...
function scrollToLatestAnswerStart(){
  ...
  if(Date.now()<visitorReadingUntil)return;
  const box=q('#chat-popover.open .chat-msgs'); if(!box)return;
  const admins=qa('.chat-bubble.admin',box); if(!admins.length)return;
  const last=admins[admins.length-1];
  const key=(last.innerText||'').slice(0,180)+'|'+(last.innerText||'').length+'|'+last.offsetTop;
  if(key===lastAnswerKey)return;
  lastAnswerKey=key;
  requestAnimationFrame(()=>{ box.scrollTop=Math.max(0,last.offsetTop-14); });
}

// Blindar scrollIntoView y scrollTop forzado solo si el visitante toca scroll.
const oldScrollInto=Element.prototype.scrollIntoView;
if(oldScrollInto&&!oldScrollInto.__tu6l){
  Element.prototype.scrollIntoView=function(){
    if(this.closest&&this.closest('#chat-popover.open .chat-msgs')&&isVisitor()&&Date.now()<visitorReadingUntil)return;
    return oldScrollInto.apply(this,arguments);
  };
  Element.prototype.scrollIntoView.__tu6l=1;
}

// 2) JAVIER: un único flujo.
function wantsJavier(text){
  const x=norm(text);
  return [
    'quiero hablar con javier','puedo hablar con javier','necesito hablar con javier','pasame con javier',
    'me pasas con javier','me podes pasar con javier','me puedes pasar con javier',
    'quiero hablar con el dueno','quiero hablar con el dueño','pasame con el dueno','pasame con el dueño',
    'puedo hablar con el fotografo','puedo hablar con el fotógrafo','me comunican con javier',
    'me puedo comunicar con javier','javier se encuentra','quiero consultar a javier',
    'llamar a javier','llama a javier','podes llamar a javier','puedes llamar a javier',
    'atencion humana','atención humana','humano','persona','asesor',
    'cel de javier','celular de javier','telefono de javier','teléfono de javier','whatsapp de javier'
  ].map(norm).some(p=>x.includes(p));
}
function hasPhone(text){return String(text||'').replace(/[^\d+]/g,'').length>=8}
function extractPhone(text){const m=String(text||'').match(/(?:\+?54)?(?:\D*\d){8,14}/); if(!m)return ''; const d=m[0].replace(/\D/g,''); return d.length>=8?d:''}
function callText(sec){
...
function playHuman(){
  safe(()=>{
    ...
    const ctx=new AC(), t=ctx.currentTime+.02;
    [620,780,980,1220,980].forEach((f,i)=>{
      const o=ctx.createOscillator(), g=ctx.createGain();
      o.frequency.value=f; g.gain.value=.38; o.connect(g); g.connect(ctx.destination);
      o.start(t+i*.14); o.stop(t+i*.14+.12);
    });
    setTimeout(()=>{if(ctx.close)ctx.close()},900);
  });
}
function startRing(id){
  if(!id||ringTimers[id])return;
...
function stopRing(id){if(ringTimers[id]&&ringTimers[id]!==true)clearTimeout(ringTimers[id]); delete ringTimers[id]}

const oldResponder=window.responderAutomaticoChat;
if(typeof oldResponder==='function'&&!oldResponder.__tu6l){
  const responder6l=async function(id,text){
    const c=chats()[id]||{};
    ...
    if(wantsJavier(text)){
      const key=id+'|'+norm(text);
      if(handledHuman[key])return;
      handledHuman[key]=true;
      const t=Date.now();
      await updateChat(id,{
        humanRequested:true,waitingHuman:false,humanFallbackSent:false,humanConfirm:false,
        priority:true,prioridad:true,unreadAdmin:true,
        callUntil:t+60000,updatedAt:t,lastUserMsg:text,lastUserAt:t,
        javierOnline:false,javierOnlineAt:0,humanMode:false
      });
      await pushMessage(id,{from:'admin',auto:true,text:callText(60),time:chatTimeSafe(),createdAt:Date.now(),systemCall:true,callUntil:t+60000});
      if(isAdmin())startRing(id);
      return;
  ...
  responder6l.__tu6l=1; window.responderAutomaticoChat=responder6l; try{responderAutomaticoChat=responder6l}catch(e){}
}

function watchHumanCalls(){
  if(!isAdmin())return;
  Object.entries(chats()).forEach(([id,c])=>{
    c=c||{};
    if(!(c.humanRequested&&c.callUntil&&!c.humanFallbackSent))return;
    if(Number(c.callUntil)<Date.now())return;
    const key=id+'|'+c.callUntil;
    if(ringingSeen[key])return;
    ringingSeen[key]=true;
    startRing(id);
  });
}
function updateCounter(){
  const id=isAdmin()?adminId():visitorId();
  const c=chats()[id]||{};
  if(!c.humanRequested||!c.callUntil||c.humanFallbackSent)return;
  const left=Math.max(0,Math.ceil((Number(c.callUntil)-Date.now())/1000));
  qa('#chat-popover.open .chat-bubble.admin').forEach(b=>{
    const t=b.innerText||'';
    if(/Aguardá un momento|intentar conectar con Javier|intentar contactar con Javier|Javier puede responder/i.test(t)){
      b.innerHTML='<div class="tu6l-white">'+callText(left).replace(/\n/g,'<br>')+'</div><div class="chat-meta">Ahora</div>';
    }
  });
}
function fallbackOnce(){
  Object.entries(chats()).forEach(([id,c])=>{
    c=c||{};
    if(!c.humanRequested||!c.callUntil||c.humanFallbackSent||fallbackSeen[id])return;
    if(Date.now()<Number(c.callUntil))return;
    fallbackSeen[id]=true; stopRing(id);
    updateChat(id,{waitingHuman:true,humanFallbackSent:true,priority:false,prioridad:false,updatedAt:Date.now()});
    pushMessage(id,{from:'admin',auto:true,text:fallbackText(),time:chatTimeSafe(),createdAt:Date.now(),humanFallback:true});
  });
}
function dedupeHuman(){
  let seenFallback=false;
  qa('#chat-popover.open .chat-bubble.admin').forEach(b=>{
    const t=b.innerText||'';
    if(/Al parecer Javier está ocupado/i.test(t)){ if(seenFallback)b.remove(); else seenFallback=true; }
    if(/Javier no pudo responder en este momento|Javier responderá personalmente/i.test(t)) b.remove();
  });
}

// 3) Header único.
function humanOnline(c){return !!(c&&c.humanMode&&c.javierOnline&&c.javierOnlineAt&&Date.now()-Number(c.javierOnlineAt)<15*60*1000)}
function updateHeader(){
  if(!isVisitor())return;
  const id=visitorId(), c=id?(chats()[id]||{}):{}, title=q('#chat-popover.open .chat-title'), sub=q('#chat-popover.open .chat-subline');
  if(!title)return;
  if(humanOnline(c)){title.textContent='JAVIER ONLINE'; if(sub)sub.innerHTML='🟢 Javier está en línea'}
  else{title.textContent='ASISTENTE TOMAUNO'; if(sub)sub.textContent='Asistente Tomauno'}
}

// 4) Estado colores.
function hasLead(c){return !!(c&&(c.humanFallbackSent||c.humanConfirm||c.whatsapp||c.phone||c.telefono||c.resumenConsulta))}
function applyStates(){
  qa('.chat-tab,.chat-list-item,.chat-inbox-item,[data-chat-id]').forEach(el=>{
    const id=el.getAttribute('data-chat-id')||el.dataset.chatId||''; if(!id)return;
    const c=chats()[id]||{};
    const red=!!(c.humanRequested&&!c.humanFallbackSent&&!c.humanConfirm);
    const yellow=!red&&!!c.unreadAdmin;
    const green=!red&&!yellow&&!!(c.userOnline||(c.userLastSeen&&Date.now()-Number(c.userLastSeen)<70000));
    el.classList.toggle('priority',red); el.classList.toggle('human',red);
    el.classList.toggle('unread',yellow); el.classList.toggle('new',yellow);
    el.classList.toggle('online',green); el.classList.toggle('off',!red&&!yellow&&!green);
    el.classList.toggle('javier-lead',hasLead(c));
    if(hasLead(c)&&!el.querySelector('.tu6l-star'))el.insertAdjacentHTML('afterbegin','<span class="tu6l-star" title="Consulta/WhatsApp para revisar">⭐</span>');
  });
}

// 5) Evitar doble Ver cursos.
function dedupeActionButtons(){
  qa('#chat-popover.open .chat-bubble.admin').forEach(b=>{
    const seen={};
    qa('button,.tu6j-action,.chat-action',b).forEach(btn=>{
      const key=(btn.innerText||btn.textContent||'').trim().toLowerCase();
      if(!key)return;
...
function css(){
  if(q('#tu6l-css'))return;
  const st=document.createElement('style'); st.id='tu6l-css';
  st.textContent=`
    .tu6l-white{background:#fff!important;color:#111!important;border-radius:18px!important;padding:12px 14px!important;box-shadow:0 8px 24px rgba(0,0,0,.22)!important;font-weight:700!important;line-height:1.45!important}
    .tu6l-white::first-line{color:#e8000a!important;font-weight:900!important}
    .tu6l-star{display:inline-flex!important;margin-right:4px!important;color:#ffd54a!important;filter:drop-shadow(0 0 5px rgba(255,213,74,.65))!important}
    .chat-tab.priority,.chat-list-item.priority,[data-chat-id].priority{border-color:#e8000a!important;box-shadow:inset 3px 0 0 rgba(232,0,10,.95)!important}
    .chat-tab.unread,.chat-list-item.unread,[data-chat-id].unread{border-color:#ffd54a!important;box-shadow:inset 3px 0 0 rgba(255,213,74,.95)!important}
    .chat-tab.javier-lead,.chat-list-item.javier-lead,[data-chat-id].javier-lead{outline:1px solid rgba(255,213,74,.35)!important}
  `;
  document.head.appendChild(st);
}
css();

setInterval(()=>{scrollToLatestAnswerStart();watchHumanCalls();updateCounter();fallbackOnce();dedupeHuman();updateHeader();applyStates();dedupeActionButtons()},700);
})();



// ─────────────────────────────────────────────────────────────────────────────
// TOMAUNO v28b FINAL 6M REPARADA — NO LIMPIEZA AGRESIVA
// Recupera conexión usando 6L como base. Solo corrige el rebote bandeja/chat.
// ─────────────────────────────────────────────────────────────────────────────
(function(){
  'use strict';

  function safe(fn){ try{return fn();}catch(e){ try{console.warn('TU final6m-repair:',e);}catch(_){} } }
  function q(s,r){ return (r||document).querySelector(s); }

  var bandejaLockUntil = 0;

  var oldPanel = window.abrirPanelChatsAdmin;
  if(typeof oldPanel === 'function' && !oldPanel.__tu6mRepair){
    var panelRepair = function(){
      bandejaLockUntil = Date.now() + 1800;
      safe(function(){
        window.currentOpenChatId = '';
        if(typeof currentOpenChatId !== 'undefined') currentOpenChatId = '';
      });
      return oldPanel.apply(this,arguments);
    };
    panelRepair.__tu6mRepair = 1;
    window.abrirPanelChatsAdmin = panelRepair;
    try{ abrirPanelChatsAdmin = panelRepair; }catch(e){}
  }

  var oldOpen = window.abrirChatAdmin;
  if(typeof oldOpen === 'function' && !oldOpen.__tu6mRepair){
    var openRepair = function(id,silent){
      if(Date.now() < bandejaLockUntil && silent) return;
      return oldOpen.apply(this,arguments);
    };
    openRepair.__tu6mRepair = 1;
    window.abrirChatAdmin = openRepair;
    try{ abrirChatAdmin = openRepair; }catch(e){}
  }

  function forceHeader(){
    var title = q('#chat-popover.open .chat-title');
    var sub = q('#chat-popover.open .chat-subline');
    var isVisitor = !!q('#chat-popover.open #chat-text') && !q('#chat-popover.open #chat-admin-text');
    if(!isVisitor || !title) return;
    ...
    var human = !!(c.humanMode && c.javierOnline && c.javierOnlineAt && Date.now() - Number(c.javierOnlineAt) < 15*60*1000);
    if(human){
      title.textContent = 'JAVIER ONLINE';
      if(sub) sub.innerHTML = '🟢 Javier está en línea';
    }else{
  ...
  setInterval(forceHeader, 700);
})();


// TOMAUNO v28b FINAL 6N — SCROLL / PANEL / RESUMEN
(function(){
'use strict';

function safe(fn){try{return fn()}catch(e){try{console.warn('TU final6n:',e)}catch(_){}}}
function q(s,r){return (r||document).querySelector(s)}
function qa(s,r){return Array.from((r||document).querySelectorAll(s))}

let visitorReadingUntil=0,lastAnswerKey='',bandejaLockUntil=0;
...
function lockVisitorReading(){if(!isVisitor())return;visitorReadingUntil=Date.now()+25000;window.__tomaunoVisitorReadingUntil=visitorReadingUntil}
['wheel','touchstart','touchmove','pointerdown','mousedown'].forEach(ev=>document.addEventListener(ev,e=>{if(e.target&&e.target.closest&&e.target.closest('#chat-popover.open .chat-msgs'))lockVisitorReading()},true));

function scrollVisitorToAnswer(){
 if(!isVisitor()||Date.now()<visitorReadingUntil)return;
 const box=q('#chat-popover.open .chat-msgs'); if(!box)return;
 const admins=qa('.chat-bubble.admin',box); if(!admins.length)return;
 const last=admins[admins.length-1], text=(last.innerText||'').trim(); if(!text)return;
 const key=text.slice(0,180)+'|'+text.length+'|'+last.offsetTop;
 if(key===lastAnswerKey)return; lastAnswerKey=key;
 setTimeout(()=>{if(Date.now()<visitorReadingUntil)return; box.scrollTop=Math.max(0,last.offsetTop-18)},80);
}

const oldScrollInto=Element.prototype.scrollIntoView;
if(oldScrollInto&&!oldScrollInto.__tu6n){
 Element.prototype.scrollIntoView=function(){
  if(this.closest&&this.closest('#chat-popover.open .chat-msgs')&&isVisitor()&&Date.now()<visitorReadingUntil)return;
  return oldScrollInto.apply(this,arguments);
 };
 Element.prototype.scrollIntoView.__tu6n=1;
}

const oldPanel=window.abrirPanelChatsAdmin;
if(typeof oldPanel==='function'&&!oldPanel.__tu6n){
 const panel6n=function(){bandejaLockUntil=Date.now()+2500;safe(()=>{window.currentOpenChatId=''; if(typeof currentOpenChatId!=='undefined')currentOpenChatId=''});return oldPanel.apply(this,arguments)};
 panel6n.__tu6n=1;window.abrirPanelChatsAdmin=panel6n;try{abrirPanelChatsAdmin=panel6n}catch(e){}
}
const oldOpen=window.abrirChatAdmin;
if(typeof oldOpen==='function'&&!oldOpen.__tu6n){
 const open6n=function(id,silent){if(Date.now()<bandejaLockUntil&&silent)return;return oldOpen.apply(this,arguments)};
 open6n.__tu6n=1;window.abrirChatAdmin=open6n;try{abrirChatAdmin=open6n}catch(e){}
}
...
function humanActive(c){return !!(c&&c.humanRequested&&!c.humanFallbackSent&&!c.humanConfirm)}
...
function applyPanelStates(){
 const db=chats(), counts={}, seen={};
 Object.values(db).forEach(c=>{const n=String(c.name||c.nombre||'').trim().toLowerCase();if(n)counts[n]=(counts[n]||0)+1});
 qa('.chat-tab,.chat-list-item,.chat-inbox-item,[data-chat-id]').forEach(el=>{
  const id=el.getAttribute('data-chat-id')||el.dataset.chatId||''; if(!id)return;
  const c=db[id]||{}, red=humanActive(c), yellow=!red&&isUnread(c), green=!red&&!yellow&&isOnline(c);
  el.classList.toggle('priority',red);el.classList.toggle('human',red);
  el.classList.toggle('unread',yellow);el.classList.toggle('new',yellow);
  el.classList.toggle('online',green);el.classList.toggle('off',!red&&!yellow&&!green);
  el.classList.toggle('javier-lead',hasLead(c));
  if(red&&!el.querySelector('.tu6n-mega'))el.insertAdjacentHTML('afterbegin','<span class="tu6n-mega" title="Llamada a Javier">📣</span>');
  if(hasLead(c)&&!el.querySelector('.tu6n-star'))el.insertAdjacentHTML('afterbegin','<span class="tu6n-star" title="Consulta/WhatsApp para revisar">⭐</span>');
  const baseName=String(c.name||c.nombre||'').trim(), key=baseName.toLowerCase();
  if(baseName&&counts[key]>1){seen[key]=(seen[key]||0)+1; const suffix=seen[key]>1?' '+seen[key]:''; const nameNode=el.querySelector('.chat-tab-name,.chat-name,strong,b'); if(nameNode&&!nameNode.dataset.tu6nName){nameNode.dataset.tu6nName='1';nameNode.textContent=baseName+suffix}}
 });
}

function extractPhone(text){const m=String(text||'').match(/(?:\+?54)?(?:\D*\d){8,14}/); if(!m)return ''; const d=m[0].replace(/\D/g,''); return d.length>=8?d:''}
const oldResponder=window.responderAutomaticoChat;
if(typeof oldResponder==='function'&&!oldResponder.__tu6n){
 const responder6n=async function(id,text){
  const c=chats()[id]||{}, phone=extractPhone(text);
  if((c.humanRequested||c.waitingHuman||c.humanFallbackSent)&&phone){
   await updateChat(id,{whatsapp:phone,telefono:phone,phone:phone,consultaJavier:true,humanRequested:false,waitingHuman:false,humanFallbackSent:false,humanConfirm:true,priority:false,prioridad:false,unreadAdmin:true,updatedAt:Date.now(),lastUserMsg:text,lastUserAt:Date.now(),resumenTema:c.resumenTema||'Consulta',resumenConsulta:text});
 ...
 responder6n.__tu6n=1;window.responderAutomaticoChat=responder6n;try{responderAutomaticoChat=responder6n}catch(e){}
}

function updateHeader(){
 if(!isVisitor())return;
 const c=chats()[visitorId()]||{}, title=q('#chat-popover.open .chat-title'), sub=q('#chat-popover.open .chat-subline'); if(!title)return;
 const human=!!(c.humanMode&&c.javierOnline&&c.javierOnlineAt&&Date.now()-Number(c.javierOnlineAt)<15*60*1000);
 if(human){title.textContent='JAVIER ONLINE';if(sub)sub.innerHTML='🟢 Javier está en línea'}else{title.textContent='ASISTENTE TOMAUNO';if(sub)sub.textContent='Asistente Tomauno'}
}

function improveFullscreen(){
 const pop=q('#chat-popover.open'); if(!pop)return;
 if(pop.classList.contains('max')||pop.classList.contains('maximized')||pop.classList.contains('fullscreen')){pop.style.left='50%';pop.style.transform='translateX(-50%)';pop.style.maxWidth='min(1180px,96vw)'}
 const inbox=q('#chat-popover.open .chat-inbox-side,#chat-popover.open .chat-inbox'); if(inbox)inbox.style.maxHeight='calc(100vh - 110px)';
}

function css(){
 if(q('#tu6n-css'))return;
 const st=document.createElement('style');st.id='tu6n-css';
 st.textContent=`
 .tu6n-star,.tu6n-mega{display:inline-flex!important;margin-right:4px!important;color:#ffd54a!important;filter:drop-shadow(0 0 5px rgba(255,213,74,.65))!important}
 .chat-tab.priority,.chat-list-item.priority,[data-chat-id].priority{border-color:#e8000a!important;box-shadow:inset 3px 0 0 rgba(232,0,10,.95)!important}
 .chat-tab.unread,.chat-list-item.unread,[data-chat-id].unread{border-color:#ffd54a!important;box-shadow:inset 3px 0 0 rgba(255,213,74,.95)!important}
 .chat-tab.online:not(.unread):not(.priority),.chat-list-item.online:not(.unread):not(.priority),[data-chat-id].online:not(.unread):not(.priority){border-color:rgba(56,210,122,.45)!important}
 .chat-tab.javier-lead,.chat-list-item.javier-lead,[data-chat-id].javier-lead{outline:1px solid rgba(255,213,74,.35)!important}`;
 document.head.appendChild(st);
}
css();
setInterval(()=>{scrollVisitorToAnswer();applyPanelStates();updateHeader();improveFullscreen()},700);
})();


// TOMAUNO v28b FINAL 6O — SCROLL / AUTO / BOTONES
(function(){
'use strict';

function safe(fn){try{return fn()}catch(e){try{console.warn('TU final6o:',e)}catch(_){}}}
function q(s,r){return (r||document).querySelector(s)}
function qa(s,r){return Array.from((r||document).querySelectorAll(s))}
function norm(s){return String(s||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9ñ\s]/g,' ').replace(/\s+/g,' ').trim()}

let visitorLockUntil=0,lastScrollKey='',lastAutoClearId='';
...
// 1) Scroll visitante correcto.
// - Si aparece pregunta del usuario: bajar al final para ver lo enviado.
// - Si aparece respuesta del asistente: ubicar al inicio de esa respuesta.
// - Si el usuario tocó el scroll: no mover por 25s.
function lockVisitor(){if(!isVisitor())return;visitorLockUntil=Date.now()+25000;window.__tomaunoVisitorReadingUntil=visitorLockUntil}
...
function smartVisitorScroll(){
 ...
 const last=bubbles[bubbles.length-1];
 const key=(last.className||'')+'|'+(last.innerText||'').slice(0,120)+'|'+(last.innerText||'').length+'|'+last.offsetTop;
...
const oldScrollInto=Element.prototype.scrollIntoView;
if(oldScrollInto&&!oldScrollInto.__tu6o){
 Element.prototype.scrollIntoView=function(){
   if(this.closest&&this.closest('#chat-popover.open .chat-msgs')&&isVisitor()&&Date.now()<visitorLockUntil)return;
   return oldScrollInto.apply(this,arguments);
 };
 Element.prototype.scrollIntoView.__tu6o=1;
}

// 2) Botones duplicados y WhatsApp extra.
function cleanActionButtons(){
 qa('#chat-popover.open .chat-bubble.admin').forEach(b=>{
   const seen={};
   qa('button,.chat-action,.tu6j-action,a',b).forEach(el=>{
     const txt=(el.innerText||el.textContent||'').trim().toLowerCase();
     const action=(el.dataset&&el.dataset.tuAction)||'';
     const key=action||txt;
     if(!key)return;

     // No agregar WhatsApp automáticamente debajo de cada texto.
     // Dejamos links internos del mensaje, pero quitamos botones generados.
     if((action==='whatsapp'||txt==='whatsapp'||txt.includes('whatsapp')) && el.classList.contains('tu6j-action')){
       el.remove(); return;
     }
...
// 3) AUTO vuelve a funcionar: limpiar modo humano del chat y dejar responder al asistente.
async function setChatAuto(id){
 if(!id)return;
 await updateChat(id,{
   humanMode:false,
   manualUntil:0,
...
async function setChatHuman(id){
 if(!id)return;
 await updateChat(id,{
   humanMode:true,
   manualUntil:Date.now()+3600000,
...
window.tomaunoToggleModoChatActual=async function(id){
 const chatId=id||adminId(); if(!chatId)return;
 const c=chats()[chatId]||{};
 const human=!!c.humanMode;
 if(human){
   await setChatAuto(chatId);
   safe(()=>toast('🤖 Este chat vuelve a automático',true));
 }else{
   await setChatHuman(chatId);
...
// Si el chat quedó marcado humano pero el botón está en AUTO, evitar que el wrapper lo mate.
const oldResponder=window.responderAutomaticoChat;
if(typeof oldResponder==='function'&&!oldResponder.__tu6o){
 const responder6o=function(id,text){
   const c=chats()[id]||{};
   if(c.autoRestoredAt && !c.humanMode){
     c.humanRequested=false; c.waitingHuman=false; c.humanFallbackSent=false;
   }
   return oldResponder.apply(this,arguments);
 };
 responder6o.__tu6o=1; window.responderAutomaticoChat=responder6o; try{responderAutomaticoChat=responder6o}catch(e){}
}

// 4) Amarillo visible y persistente mientras no leído.
// Forzamos estilo inline porque otros renderizadores reescriben clases.
function getItemId(el){
 return el.getAttribute('data-chat-id')||el.dataset.chatId||'';
}
function isUnread(c){return !!(c&&(c.unreadAdmin||c.hasNewAdmin||c.hasNew||c.unread))}
...
function applyStrongStates(){
 qa('.chat-tab,.chat-list-item,.chat-inbox-item,[data-chat-id]').forEach(el=>{
   ...
   if(red&&!el.querySelector('.tu6o-mega'))el.insertAdjacentHTML('afterbegin','<span class="tu6o-mega" title="Llamada a Javier">📣</span>');
   if(hasLead(c)&&!el.querySelector('.tu6o-star'))el.insertAdjacentHTML('afterbegin','<span class="tu6o-star" title="Consulta/WhatsApp para revisar">⭐</span>');
 });
}

// 5) Nombres duplicados visuales robustos.
function duplicateNames(){
 const db=chats(), counts={}, seen={};
 Object.entries(db).forEach(([id,c])=>{const n=String(c.name||c.nombre||'').trim().toLowerCase();if(n)counts[n]=(counts[n]||0)+1});
 qa('.chat-tab,.chat-list-item,.chat-inbox-item,[data-chat-id]').forEach(el=>{
   const id=getItemId(el); if(!id)return;
   const c=db[id]||{}, n=String(c.name||c.nombre||'').trim(), k=n.toLowerCase();
   if(!n||counts[k]<=1)return;
   seen[k]=(seen[k]||0)+1;
   const suffix=seen[k]>1?' '+seen[k]:'';
   const node=el.querySelector('.chat-tab-name,.chat-name,strong,b')||el.firstElementChild;
   if(node&&node.dataset.tu6oName!==n+suffix){
     node.dataset.tu6oName=n+suffix;
...
function updateHeader(){
 ...
 const c=chats()[visitorId()]||{}, title=q('#chat-popover.open .chat-title'), sub=q('#chat-popover.open .chat-subline'); if(!title)return;
 const human=!!(c.humanMode&&c.javierOnline&&c.javierOnlineAt&&Date.now()-Number(c.javierOnlineAt)<15*60*1000);
 if(human){title.textContent='JAVIER ONLINE'; if(sub)sub.innerHTML='🟢 Javier está en línea'}
 else{title.textContent='ASISTENTE TOMAUNO'; if(sub)sub.textContent='Asistente Tomauno'}
}

function css(){
 if(q('#tu6o-css'))return;
 const st=document.createElement('style'); st.id='tu6o-css';
 st.textContent=`
 .tu6o-star,.tu6o-mega{display:inline-flex!important;margin-right:4px!important;color:#ffd54a!important;filter:drop-shadow(0 0 5px rgba(255,213,74,.65))!important}
 .chat-tab.unread,.chat-list-item.unread,[data-chat-id].unread{border-color:#ffd54a!important;box-shadow:inset 3px 0 0 rgba(255,213,74,.95)!important}
 .chat-tab.priority,.chat-list-item.priority,[data-chat-id].priority{border-color:#e8000a!important;box-shadow:inset 3px 0 0 rgba(232,0,10,.95)!important}`;
 document.head.appendChild(st);
}
...
setInterval(()=>{smartVisitorScroll();cleanActionButtons();applyStrongStates();duplicateNames();updateHeader()},650);
})();
...
// TOMAUNO v28b FINAL 6U — ESTABILIDAD UI
(function(){
'use strict';

function safe(fn){try{return fn()}catch(e){try{console.warn('TU final6u:',e)}catch(_){}}}
function q(s,r){return (r||document).querySelector(s)}
function qa(s,r){return Array.from((r||document).querySelectorAll(s))}

let lastVisitorKey='';
let adminOpenLockUntil=0;
let lastFabClick=0;
...
// Scroll visitante: simple y estable, siempre al final cuando aparece una burbuja nueva.
function forceVisitorBottom(){
 ...
 setTimeout(()=>{box.scrollTop=box.scrollHeight},260);
}

// Blindaje scrollIntoView: evita quedar en medio.
const oldScrollInto=Element.prototype.scrollIntoView;
if(oldScrollInto&&!oldScrollInto.__tu6u){
 Element.prototype.scrollIntoView=function(){
  if(this.closest&&this.closest('#chat-popover.open .chat-msgs')&&isVisitor()){
   setTimeout(()=>{const box=q('#chat-popover.open .chat-msgs'); if(box)box.scrollTop=box.scrollHeight},60);
  }
  return oldScrollInto.apply(this,arguments);
 };
 Element.prototype.scrollIntoView.__tu6u=1;
}

function stableRows(){
 qa('.chat-tab,.chat-list-item,.chat-inbox-item,[data-chat-id]').forEach(row=>row.classList.add('tu6u-stable-row'));
}

function applyStableStates(){
 qa('.chat-tab,.chat-list-item,.chat-inbox-item,[data-chat-id]').forEach(el=>{
  const id=getItemId(el);
  const c=id?(chats()[id]||{}):{};
  const txt=(el.innerText||'').toLowerCase();

  const red=!!(c.humanRequested&&!c.humanFallbackSent&&!c.humanConfirm) || (/atenci[oó]n javier|humano|llamada|javier/.test(txt) && /esperando|prioridad/.test(txt));
  const yellow=!red && (!!(c.unreadAdmin||c.hasNewAdmin||c.hasNew||c.unread) || /esperando|pendiente|sin leer/.test(txt));
  const green=!red&&!yellow&&(!!(c.userOnline||c.online||c.isOnline||(c.userLastSeen&&Date.now()-Number(c.userLastSeen)<70000)) || /online|en l[ií]nea/.test(txt));

  el.classList.toggle('tu6u-red',red);
  el.classList.toggle('tu6u-yellow',yellow);
  el.classList.toggle('tu6u-green',green);

  if(red&&!el.querySelector('.tu6u-mega'))el.insertAdjacentHTML('afterbegin','<span class="tu6u-mega" title="Llamada / Atención Javier">📣</span>');
  const lead=!!(c.humanFallbackSent||c.humanConfirm||c.whatsapp||c.phone||c.telefono||c.resumenConsulta||c.consultaJavier)||/whatsapp|consulta/i.test(txt);
  if(lead&&!el.querySelector('.tu6u-star'))el.insertAdjacentHTML('afterbegin','<span class="tu6u-star" title="Consulta/WhatsApp pendiente">⭐</span>');
 });
}
...
function markDuplicates(){
 ...
 qa('.chat-tab,.chat-list-item,.chat-inbox-item,[data-chat-id]').forEach(el=>{
  ...
  const num=order[id]||1;
  if(num<=1)return;
  if(!el.querySelector('.tu6u-dupe')){
   const badge=document.createElement('span');
   badge.className='tu6u-dupe';
...
// Reducir abre/cierra por doble clic rápido.
const oldPanel=window.abrirPanelChatsAdmin;
if(typeof oldPanel==='function'&&!oldPanel.__tu6u){
 const panel6u=function(){
  adminOpenLockUntil=Date.now()+1200;
  return oldPanel.apply(this,arguments);
 };
 panel6u.__tu6u=1;
 window.abrirPanelChatsAdmin=panel6u;
 try{abrirPanelChatsAdmin=panel6u}catch(e){}
}
const oldClose=window.cerrarChatPopover;
if(typeof oldClose==='function'&&!oldClose.__tu6u){
 const close6u=function(){
  if(Date.now()<adminOpenLockUntil&&isAdmin())return;
  return oldClose.apply(this,arguments);
 };
 close6u.__tu6u=1;
 window.cerrarChatPopover=close6u;
 try{cerrarChatPopover=close6u}catch(e){}
}
document.addEventListener('click',e=>{
 const fab=e.target&&e.target.closest&&e.target.closest('#chat-fab,.chat-fab,[data-chat-fab]');
 if(!fab)return;
 const now=Date.now();
 if(now-lastFabClick<450){
  e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();return false;
 }
 lastFabClick=now;
},true);

function improveInbox(){
 const pop=q('#chat-popover.open'); if(!pop)return;
 const inbox=q('#chat-popover.open .chat-inbox-full,#chat-popover.open .chat-inbox-list,#chat-popover.open .chat-inbox-side');
 const isInbox=!!inbox&&!q('#chat-popover.open #chat-admin-text');
 pop.classList.toggle('tu6u-inbox-mode',isInbox);
}

function buttonIntent(el){
...
function css(){
 if(q('#tu6u-css'))return;
 const st=document.createElement('style');
 st.id='tu6u-css';
 st.textContent=`
 .tu6u-stable-row{min-height:58px!important;height:auto!important;padding-top:9px!important;padding-bottom:9px!important;transition:none!important;transform:none!important;will-change:auto!important}
 .tu6u-stable-row *{transition:none!important}
 .tu6u-red{border-color:#e8000a!important;box-shadow:inset 3px 0 0 rgba(232,0,10,.95)!important}
 .tu6u-yellow{border-color:#ffd54a!important;box-shadow:inset 3px 0 0 rgba(255,213,74,.95)!important}
 .tu6u-green:not(.tu6u-yellow):not(.tu6u-red){border-color:rgba(56,210,122,.55)!important;box-shadow:none!important}
 .tu6u-star,.tu6u-mega{display:inline-flex!important;margin-right:4px!important;color:#ffd54a!important;filter:drop-shadow(0 0 5px rgba(255,213,74,.65))!important}
 .tu6u-dupe{display:inline-flex!important;align-items:center!important;justify-content:center!important;min-width:22px!important;height:18px!important;border-radius:99px!important;background:#e8000a!important;color:#fff!important;font-size:11px!important;margin-left:5px!important;padding:0 6px!important;vertical-align:middle!important}
 #chat-popover.open.tu6u-inbox-mode{width:min(900px,96vw)!important;max-width:min(900px,96vw)!important;left:50%!important;transform:translateX(-50%)!important}
 #chat-popover.open.tu6u-inbox-mode .chat-inbox-list,#chat-popover.open.tu6u-inbox-mode .chat-inbox-full,#chat-popover.open.tu6u-inbox-mode .chat-inbox-side{width:100%!important;max-width:none!important;max-height:calc(92vh - 140px)!important;overflow:auto!important}
 `;
 document.head.appendChild(st);
}
css();

setInterval(()=>{forceVisitorBottom();stableRows();applyStableStates();markDuplicates();improveInbox();dedupeButtons()},650);
})();


// TOMAUNO v28b FINAL 6V — SILENCIO REAL DE ALARMA
// No toca cerebro ni respuestas. Solo corta audio de alarmas cuando el ADM atiende.
(function(){
'use strict';

function safe(fn){try{return fn()}catch(e){try{console.warn('TU final6v:',e)}catch(_){}}}
function q(s,r){return (r||document).querySelector(s)}
function qa(s,r){return Array.from((r||document).querySelectorAll(s))}
...
function muteAlarms(ms){
  window.tomaunoMuteAlarmsUntil = Date.now() + (ms || 120000);
...
// 1) Silenciar osciladores WebAudio mientras esté muteado.
// Esto corta incluso alarmas viejas que quedaron con setTimeout activo.
safe(()=>{
  if(window.__tu6vOscillatorPatch) return;
  window.__tu6vOscillatorPatch = true;
...
// 2) Silenciar audio HTML si alguna alarma usa <audio>.
safe(()=>{
  if(window.__tu6vAudioPatch) return;
  window.__tu6vAudioPatch = true;
...
// 3) Al abrir chat ADM, silenciar. Es lo que querías: llego a la PC, abro el chat, deja de sonar.
const oldOpenAdmin = window.abrirChatAdmin;
if(typeof oldOpenAdmin === 'function' && !oldOpenAdmin.__tu6v){
  const open6v = function(id, silent){
    const r = oldOpenAdmin.apply(this, arguments);
    if(id) muteAlarms(120000);
    return r;
  };
  open6v.__tu6v = 1;
  window.abrirChatAdmin = open6v;
  try{ abrirChatAdmin = open6v; }catch(e){}
}

// 4) Al responder desde ADM, silenciar.
const oldSendAdmin = window.enviarChatAdmin;
if(typeof oldSendAdmin === 'function' && !oldSendAdmin.__tu6v){
  const send6v = async function(){
    muteAlarms(120000);
    return oldSendAdmin.apply(this, arguments);
  };
  send6v.__tu6v = 1;
  window.enviarChatAdmin = send6v;
  try{ enviarChatAdmin = send6v; }catch(e){}
}

// 5) Al pasar a HUM/AUTO, silenciar.
const oldToggle = window.tomaunoToggleModoChatActual;
if(typeof oldToggle === 'function' && !oldToggle.__tu6v){
  const toggle6v = function(){
    muteAlarms(120000);
    return oldToggle.apply(this, arguments);
  };
  toggle6v.__tu6v = 1;
  window.tomaunoToggleModoChatActual = toggle6v;
}

// 6) Botón visible 🔕 en herramientas ADM.
function addMuteButton(){
  if(!isAdmin()) return;
  const tools = q('#chat-popover.open .chat-admin-tools') || q('#chat-popover.open .chat-tools') || q('#chat-popover.open .admin-tools');
  if(!tools || tools.querySelector('.tu6v-mute')) return;
  const btn = document.createElement('button');
  btn.type = 'button';
  btn.className = 'tu6v-mute';
  btn.textContent = '🔕';
  btn.title = 'Silenciar alarmas por 2 minutos';
  btn.setAttribute('aria-label','Silenciar alarmas');
  btn.addEventListener('click', ev=>{
    ev.preventDefault();
    ev.stopPropagation();
    muteAlarms(120000);
    safe(()=>toast('🔕 Alarmas silenciadas por 2 minutos', true));
  }, true);
  tools.appendChild(btn);
}

// 7) Si el ADM está mirando un chat con llamada, también silenciar automáticamente.
function autoMuteWhenViewingCall(){
  if(!isAdmin()) return;
  const id = adminId();
  if(!id) return;
  const txt = q('#chat-popover.open .chat-msgs')?.innerText || '';
  if(/Aguardá un momento|intentar conectar con Javier|llamada para Javier/i.test(txt)){
    muteAlarms(120000);
  }
}

function css(){
  if(q('#tu6v-css')) return;
  const st = document.createElement('style');
  st.id = 'tu6v-css';
  st.textContent = `
    .tu6v-mute{
...
setInterval(()=>{addMuteButton();autoMuteWhenViewingCall()},700);
})();


// TOMAUNO v28b FINAL 6W — ESTABILIZACIÓN CHAT
(function(){
'use strict';
function safe(fn){try{return fn()}catch(e){try{console.warn('TU final6w:',e)}catch(_){}}}
function q(s,r){return (r||document).querySelector(s)}
function qa(s,r){return Array.from((r||document).querySelectorAll(s))}
function chats(){return safe(()=>window.chatsDB||chatsDB||{})||{}}
function isVisitor(){return !!q('#chat-popover.open #chat-text')&&!q('#chat-popover.open #chat-admin-text')}
...
let lastVisitorKey='', lastPanelSig='', visitorManualUntil=0;

function lockVisitor(){if(!isVisitor())return;visitorManualUntil=Date.now()+12000;window.__tomaunoVisitorReadingUntil=visitorManualUntil}
['wheel','touchstart','touchmove','pointerdown','mousedown'].forEach(ev=>document.addEventListener(ev,e=>{if(e.target&&e.target.closest&&e.target.closest('#chat-popover.open .chat-msgs'))lockVisitor()},true));

function visitorScrollBottom(){
 if(!isVisitor()||Date.now()<visitorManualUntil)return;
 const box=q('#chat-popover.open .chat-msgs'); if(!box)return;
 const bubbles=qa('.chat-bubble',box); if(!bubbles.length)return;
 const last=bubbles[bubbles.length-1];
 const key=(last.className||'')+'|'+(last.innerText||'').slice(0,140)+'|'+(last.innerText||'').length+'|'+last.offsetTop;
 if(key===lastVisitorKey)return; lastVisitorKey=key;
 requestAnimationFrame(()=>{if(Date.now()<visitorManualUntil)return;box.scrollTop=box.scrollHeight});
 setTimeout(()=>{if(Date.now()<visitorManualUntil)return;box.scrollTop=box.scrollHeight},180);
}

const oldScrollInto=Element.prototype.scrollIntoView;
if(oldScrollInto&&!oldScrollInto.__tu6w){
 Element.prototype.scrollIntoView=function(){
  if(this.closest&&this.closest('#chat-popover.open .chat-msgs')&&isVisitor()){
   setTimeout(()=>{const box=q('#chat-popover.open .chat-msgs');if(box&&Date.now()>=visitorManualUntil)box.scrollTop=box.scrollHeight},80);
  }
  return oldScrollInto.apply(this,arguments);
 };
 Element.prototype.scrollIntoView.__tu6w=1;
}

function panelSig(){
 return qa('.chat-tab,.chat-list-item,.chat-inbox-item,[data-chat-id]').map(el=>{
  const id=getId(el), c=id?(chats()[id]||{}):{};
  return [id,!!c.humanRequested,!!c.humanFallbackSent,!!c.humanConfirm,!!c.unreadAdmin,!!c.whatsapp,!!c.phone,!!c.telefono,!!c.userOnline,c.userLastSeen||''].join(':');
 }).join('|');
}
function stabilizePanel(){
 const rows=qa('.chat-tab,.chat-list-item,.chat-inbox-item,[data-chat-id]');
 rows.forEach(row=>row.classList.add('tu6w-row'));
 const sig=panelSig(); if(sig===lastPanelSig)return; lastPanelSig=sig;
 const db=chats(), groups={}, order={};
 Object.entries(db).forEach(([id,c])=>{const n=String(c.name||c.nombre||'').trim().toLowerCase();if(!n)return;if(!groups[n])groups[n]=[];groups[n].push(id)});
 Object.keys(groups).forEach(k=>groups[k].forEach((id,i)=>order[id]=i+1));
 rows.forEach(el=>{
  ...
  const unread=!!(c.unreadAdmin||c.hasNewAdmin||c.hasNew||c.unread);
  const online=!!(c.userOnline||c.online||c.isOnline||(c.userLastSeen&&Date.now()-Number(c.userLastSeen)<70000));
  el.classList.toggle('tu6w-call',call);
  el.classList.toggle('tu6w-unread',!call&&unread);
  el.classList.toggle('tu6w-online',!call&&!unread&&online);
  el.classList.toggle('tu6w-lead',lead);
  if(call&&!el.querySelector('.tu6w-mega'))el.insertAdjacentHTML('afterbegin','<span class="tu6w-mega" title="Llamada a Javier">📣</span>');
  if(lead&&!el.querySelector('.tu6w-star'))el.insertAdjacentHTML('afterbegin','<span class="tu6w-star" title="Consulta/WhatsApp pendiente">⭐</span>');
  const n=String(c.name||c.nombre||'').trim().toLowerCase(), num=order[id]||1;
  if(n&&groups[n]&&groups[n].length>1&&num>1&&!el.querySelector('.tu6w-dupe')){
   const badge=document.createElement('span');badge.className='tu6w-dupe';badge.textContent='#'+num;badge.title='Nombre repetido';
   const node=el.querySelector('.chat-tab-name,.chat-name,strong,b,.name')||el.firstElementChild||el;node.appendChild(badge);
  }
 });
}

function stableInbox(){
 const pop=q('#chat-popover.open'); if(!pop)return;
 const inbox=q('#chat-popover.open .chat-inbox-full,#chat-popover.open .chat-inbox-list,#chat-popover.open .chat-inbox-side');
 const inboxMode=!!inbox&&!q('#chat-popover.open #chat-admin-text');
 pop.classList.toggle('tu6w-inbox-mode',inboxMode);
 if(isAdmin()&&(pop.classList.contains('max')||pop.classList.contains('maximized')||pop.classList.contains('fullscreen')||pop.classList.contains('is-maximized'))){
  pop.style.left='50%';pop.style.right='auto';pop.style.top='50%';pop.style.bottom='auto';pop.style.transform='translate(-50%,-50%)';pop.style.width='min(1180px,96vw)';pop.style.maxWidth='min(1180px,96vw)';pop.style.height='min(820px,94vh)';pop.style.maxHeight='94vh';
 }
}

function intent(el){
 const text=(el.innerText||el.textContent||'').toLowerCase(), href=(el.getAttribute&&el.getAttribute('href')||'').toLowerCase(), ds=(el.dataset&&(el.dataset.tuAction||el.dataset.action||el.dataset.target)||'').toLowerCase();
 const all=text+' '+href+' '+ds;
 if(/servicio|servicios/.test(all))return 'servicios';
 if(/curso|cursos/.test(all))return 'cursos';
 if(/evento|eventos/.test(all))return 'eventos';
...
function dedupeActions(){
 qa('#chat-popover.open .chat-bubble.admin').forEach(b=>{
  const seen={};
  qa('button,.chat-action,.tu6j-action,a',b).forEach(el=>{const k=intent(el);if(!k)return;if(seen[k])el.remove();else seen[k]=true});
 });
}
function trashOnlyAdmin(){
 if(!isVisitor())return;
 qa('#chat-popover.open .chat-bubble .tu6p-del,#chat-popover.open .chat-bubble .tu-msg-del,#chat-popover.open .chat-bubble button').forEach(btn=>{const t=(btn.innerText||btn.textContent||'').trim();if(t==='🗑️'||/borrar|eliminar/i.test(btn.title||''))btn.remove()});
}
function css(){
 if(q('#tu6w-css'))return;
 const st=document.createElement('style');st.id='tu6w-css';
 st.textContent=`#chat-popover.open .tu6w-row{min-height:58px!important;height:auto!important;padding-top:9px!important;padding-bottom:9px!important;transition:none!important;transform:none!important;will-change:auto!important;contain:layout paint!important}#chat-popover.open .tu6w-row *{transition:none!important;animation:none!important}.tu6w-call{border-color:#e8000a!important;box-shadow:inset 3px 0 0 rgba(232,0,10,.95)!important}.tu6w-unread{border-color:#ffd54a!important;box-shadow:inset 3px 0 0 rgba(255,213,74,.95)!important}.tu6w-online:not(.tu6w-unread):not(.tu6w-call){border-color:rgba(56,210,122,.55)!important;box-shadow:none!important}.tu6w-lead{outline:1px solid rgba(255,213,74,.35)!important}.tu6w-star,.tu6w-mega{display:inline-flex!important;margin-right:4px!important;color:#ffd54a!important;filter:drop-shadow(0 0 5px rgba(255,213,74,.65))!important}.tu6w-dupe{display:inline-flex!important;align-items:center!important;justify-content:center!important;min-width:22px!important;height:18px!important;border-radius:99px!important;background:#e8000a!important;color:#fff!important;font-size:11px!important;margin-left:5px!important;padding:0 6px!important;vertical-align:middle!important}#chat-popover.open.tu6w-inbox-mode{width:min(900px,96vw)!important;max-width:min(900px,96vw)!important;left:50%!important;transform:translateX(-50%)!important}#chat-popover.open.tu6w-inbox-mode .chat-inbox-list,#chat-popover.open.tu6w-inbox-mode .chat-inbox-full,#chat-popover.open.tu6w-inbox-mode .chat-inbox-side{width:100%!important;max-width:none!important;max-height:calc(92vh - 140px)!important;overflow:auto!important}#chat-popover.open:not(:has(#chat-admin-text)) .tu6p-del,#chat-popover.open:not(:has(#chat-admin-text)) .tu-msg-del{display:none!important}`;
 document.head.appendChild(st);
}
css();
setInterval(()=>{visitorScrollBottom();stabilizePanel();stableInbox();dedupeActions();trashOnlyAdmin()},700);
})();
