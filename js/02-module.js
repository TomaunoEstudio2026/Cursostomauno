...
function scrollChatSmart(box){
  if(!box) return;
  const isVisitor = !!document.querySelector('#chat-popover.open #chat-text') && !document.querySelector('#chat-popover.open #chat-admin-text');
  if(isVisitor){
    const distance = box.scrollHeight - box.clientHeight - box.scrollTop;
    if((window.__tomaunoVisitorReadingUntil && Date.now() < window.__tomaunoVisitorReadingUntil) || distance > 120) return;
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// TOMAUNO v28b FINAL 6M — LIMPIEZA REAL
// Un solo bloque: Javier, scroll visitante, estados, colores, estrella/megáfono.
// ─────────────────────────────────────────────────────────────────────────────
(function(){
'use strict';

function safe(fn){try{return fn()}catch(e){try{console.warn('TU final6m:',e)}catch(_){}}}
...
function play(kind){
 safe(()=>{
  ...
  const freqs=kind==='human'?[620,780,980,1220,980]:[880,1120,880];
  const gain=kind==='human'?.38:.18;
  freqs.forEach((f,i)=>{const o=ctx.createOscillator(),g=ctx.createGain();o.frequency.value=f;g.gain.value=gain;o.connect(g);g.connect(ctx.destination);o.start(t+i*.14);o.stop(t+i*.14+.12)});
  setTimeout(()=>{if(ctx.close)ctx.close()},900);
 });
}

// NOTIFICACIONES: bloquear mensaje web normal.
const prevNotify=window.notifyAdminChat||((typeof notifyAdminChat!=='undefined')?notifyAdminChat:null);
window.notifyAdminChat=function(title,body,chatId){
 const t=String(title||''), b=String(body||'');
 if(/Nuevo mensaje web/i.test(t))return;
 const isHuman=/llamada|javier|humana/i.test(t+' '+b);
 const isNew=/nuevo chat|chat nuevo/i.test(t);
 if(!isHuman&&!isNew)return;
 if(typeof prevNotify==='function')return prevNotify.apply(this,arguments);
};
try{notifyAdminChat=window.notifyAdminChat}catch(e){}

// Solo sonido normal cuando aparece un chat realmente nuevo.
function firstUserMsg(c){
 const ms=c&&c.messages?c.messages:{};
 let best=null;
 Object.keys(ms).forEach(k=>{const m=ms[k]||{}; if(m.from==='user'&&!m.typing&&String(m.text||'').trim()){if(!best||Number(m.createdAt||0)<Number(best.createdAt||0))best=m}});
 return best;
}
function notifyNewChatOnly(){
 if(!isAdmin())return;
 const dbs=chats();
 if(knownChats===null){knownChats={};Object.keys(dbs).forEach(id=>knownChats[id]=true);return;}
 Object.entries(dbs).forEach(([id,c])=>{
  if(knownChats[id])return;
  const first=firstUserMsg(c);
...
// SCROLL visitante: si el usuario toca el scroll, no mover. Si llega respuesta nueva, mostrar inicio de la respuesta.
function lockReading(){if(!isVisitor())return;visitorReadingUntil=Date.now()+30000;window.__tomaunoVisitorReadingUntil=visitorReadingUntil}
['wheel','touchstart','touchmove','pointerdown','mousedown','scroll'].forEach(ev=>document.addEventListener(ev,e=>{if(e.target&&e.target.closest&&e.target.closest('#chat-popover.open .chat-msgs'))lockReading()},true));

const oldScrollInto=Element.prototype.scrollIntoView;
if(oldScrollInto&&!oldScrollInto.__tu6m){
 ...
 Element.prototype.scrollIntoView.__tu6m=1;
}
...
function wantsJavier(text){
 const x=norm(text);
 return [
  'quiero hablar con javier','puedo hablar con javier','necesito hablar con javier','pasame con javier','me pasas con javier',
  'quiero hablar con el dueno','quiero hablar con el dueño','pasame con el dueno','pasame con el dueño',
  'puedo hablar con el fotografo','puedo hablar con el fotógrafo','me comunican con javier','me puedo comunicar con javier',
  'javier se encuentra','quiero consultar a javier','llamar a javier','llama a javier','podes llamar a javier','puedes llamar a javier',
  'atencion humana','atención humana','humano','persona','asesor','cel de javier','celular de javier','telefono de javier','teléfono de javier','whatsapp de javier'
...
function callText(sec){sec=Math.max(0,Number(sec||0));return '📞 Aguardá un momento por favor, voy a intentar contactar con Javier para que te responda por acá 😊\n\n⏱️ '+String(sec).padStart(2,'0')+'s para intentar conectar con Javier'}
...
function confirmText(){return '✅ Perfecto. Ya quedó registrada tu consulta.\n\nApenas pueda la revisa y te responde por WhatsApp.\n\nMuchas gracias.'}
function startRing(id){
 if(!id||ringTimers[id])return;
 const started=Date.now();
 function tick(){if(!ringTimers[id])return;if(Date.now()-started>60000){stopRing(id);return}play('human');ringTimers[id]=setTimeout(tick,10000)}
...
if(typeof oldResponder==='function'&&!oldResponder.__tu6m){
 const responder6m=async function(id,text){
  const c=chats()[id]||{};
  const phone=extractPhone(text);
  if((c.humanRequested||c.waitingHuman||c.humanFallbackSent)&&phone){
   stopRing(id);
   await updateChat(id,{whatsapp:phone,telefono:phone,phone:phone,humanRequested:false,waitingHuman:false,humanFallbackSent:false,humanConfirm:true,priority:false,prioridad:false,unreadAdmin:true,updatedAt:Date.now(),lastUserMsg:text,lastUserAt:Date.now(),resumenTema:c.resumenTema||'Consulta',resumenConsulta:text});
  ...
  if(wantsJavier(text)){
   ...
   if(humanHandled[key])return;
   humanHandled[key]=true;
   const t=Date.now();
   await updateChat(id,{humanRequested:true,waitingHuman:false,humanFallbackSent:false,humanConfirm:false,priority:true,prioridad:true,unreadAdmin:true,callUntil:t+60000,updatedAt:t,lastUserMsg:text,lastUserAt:t,javierOnline:false,javierOnlineAt:0,humanMode:false});
 ...
 responder6m.__tu6m=1;window.responderAutomaticoChat=responder6m;try{responderAutomaticoChat=responder6m}catch(e){}
}

function watchHumanRings(){
 if(!isAdmin())return;
 Object.entries(chats()).forEach(([id,c])=>{
  c=c||{};
  if(c.humanRequested&&c.callUntil&&!c.humanFallbackSent&&Number(c.callUntil)>Date.now())startRing(id);
 });
}
function updateCounter(){
 ...
 qa('#chat-popover.open .chat-bubble.admin').forEach(b=>{
  ...
  if(/Aguardá un momento|intentar conectar con Javier|intentar contactar con Javier|Javier puede responder/i.test(t)){
   b.innerHTML='<div class="tu6m-white">'+callText(left).replace(/\n/g,'<br>')+'</div><div class="chat-meta">Ahora</div>';
  }
 });
}
...
window.tomaunoToggleModoChatActual=async function(id){
 ...
 const c=chats()[chatId]||{},toHuman=!c.humanMode;
 await updateChat(chatId,toHuman?{humanMode:true,manualUntil:Date.now()+3600000,javierOnline:true,javierOnlineAt:Date.now(),humanRequested:false,waitingHuman:false,priority:false,prioridad:false,unreadAdmin:false}:{humanMode:false,manualUntil:0,javierOnline:false,javierOnlineAt:0,humanRequested:false,waitingHuman:false,priority:false,prioridad:false,unreadAdmin:false});
 safe(()=>toast(toHuman?'👤 Este chat queda en humano':'🤖 Este chat vuelve a automático',true));
 setTimeout(()=>safe(()=>abrirChatAdmin(chatId,true)),80);
};
window.toggleModoAsistenteChat=function(){const id=adminId();if(id&&q('#chat-popover.open #chat-admin-text'))return window.tomaunoToggleModoChatActual(id);safe(()=>toast('Abrí un chat para pasarlo a humano/auto.',true))};

const oldSendAdmin=window.enviarChatAdmin;
if(typeof oldSendAdmin==='function'&&!oldSendAdmin.__tu6m){
 const send6m=async function(){const id=adminId();const r=await oldSendAdmin.apply(this,arguments);if(id)await updateChat(id,{humanMode:true,manualUntil:Date.now()+3600000,javierOnline:true,javierOnlineAt:Date.now(),humanRequested:false,waitingHuman:false,priority:false,prioridad:false,unreadAdmin:false});return r};
 send6m.__tu6m=1;window.enviarChatAdmin=send6m;try{enviarChatAdmin=send6m}catch(e){}
}
...
function applyStates(){
 qa('.chat-tab,.chat-list-item,.chat-inbox-item,[data-chat-id]').forEach(el=>{
  ...
  if(red&&!el.querySelector('.tu6m-mega'))el.insertAdjacentHTML('afterbegin','<span class="tu6m-mega" title="Llamada a Javier">📣</span>');
  if(hasLead(c)&&!el.querySelector('.tu6m-star'))el.insertAdjacentHTML('afterbegin','<span class="tu6m-star" title="Consulta/WhatsApp para revisar">⭐</span>');
 });
}

// Cerebro puntual
const oldBuscar=(typeof buscarRespuestaAsistente==='function')?buscarRespuestaAsistente:null;
if(oldBuscar&&!oldBuscar.__tu6m){
 const buscar6m=function(text){const x=norm(text);
  if(/siguen abiertas|inscripciones abiertas|inscripcion abierta|inscripción abierta|puedo inscribirme todavia|puedo inscribirme todavía|todavia puedo inscribirme|todavía puedo inscribirme|hay lugar|hay cupo|quedan cupos/.test(x))return 'Las inscripciones pueden variar según el curso o evento que te interese.\n\nDecime cuál querés hacer y te confirmo si sigue abierto, o tocá **Ver cursos** para revisar las opciones activas.';
  return oldBuscar.apply(this,arguments);
 };
 buscar6m.__tu6m=1;buscarRespuestaAsistente=buscar6m;try{window.buscarRespuestaAsistente=buscar6m}catch(e){}
}

function dedupeButtons(){
 qa('#chat-popover.open .chat-bubble.admin').forEach(b=>{
  const seen={};
  qa('button,.chat-action,.tu6j-action',b).forEach(btn=>{const k=(btn.innerText||btn.textContent||'').trim().toLowerCase();if(!k)return;if(seen[k])btn.remove();else seen[k]=true});
 });
}

function css(){
 if(q('#tu6m-css'))return;
 const st=document.createElement('style');st.id='tu6m-css';
 st.textContent=`
  .tu6m-white{background:#fff!important;color:#111!important;border-radius:18px!important;padding:12px 14px!important;box-shadow:0 8px 24px rgba(0,0,0,.22)!important;font-weight:700!important;line-height:1.45!important}
  .tu6m-white::first-line{color:#e8000a!important;font-weight:900!important}
  .tu6m-star,.tu6m-mega{display:inline-flex!important;margin-right:4px!important;color:#ffd54a!important;filter:drop-shadow(0 0 5px rgba(255,213,74,.65))!important}
...
setInterval(()=>{notifyNewChatOnly();scrollToLatestAnswer();watchHumanRings();updateCounter();fallbackOnce();dedupeHumanMessages();updateHeader();applyStates();dedupeButtons()},700);
})();
