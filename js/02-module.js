...
try {
  ...
  onValue(ref(db, 'tomauno/presence'), snap => {
    ...
    const vals = Object.values(presenceDB);
...
function lastSeenText(c){
  if(isChatUserOnline(c)) return 'En lÃ­nea ahora';
  const t = Number(c?.userLastSeen || 0);
  if(!t) return 'Sin actividad reciente';
  const mins = Math.max(1, Math.round((Date.now() - t)/60000));
  return mins <= 1 ? 'Se fue hace instantes' : 'Ãšltima actividad hace ' + mins + ' min';
}
...
const VISITOR_INVITE_DEFAULT = 'Hola, soy Javier de Tomauno. Si necesitas ayuda con algun curso o servicio, aqui estoy para responderte.';
let currentVisitorInvite = null;

function activeVisitorPresenceEntries(){
  const now = Date.now();
  return Object.entries(presenceDB || {}).filter(([id,v]) =>
    id !== PRESENCE_ID &&
    v &&
    v.online &&
    !v.admin &&
...
function visitorInviteStyle(){
  if(document.getElementById('visitor-invite-style')) return;
  const st = document.createElement('style');
  st.id = 'visitor-invite-style';
  st.textContent = [
    '.visitor-invite-toast{position:fixed;right:22px;bottom:112px;z-index:99998;width:min(340px,calc(100vw - 28px));background:#151515;border:1px solid rgba(232,0,10,.75);border-radius:16px;box-shadow:0 18px 40px rgba(0,0,0,.55);padding:14px;color:#fff;font-family:inherit;}',
    '.visitor-invite-toast strong{display:block;color:#fff;font-size:13px;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px;}',
    '.visitor-invite-toast p{margin:0;color:rgba(255,255,255,.86);font-size:13px;line-height:1.35;}',
    '.visitor-invite-actions{display:flex;gap:8px;margin-top:12px;}',
    '.visitor-invite-actions button{border:0;border-radius:999px;padding:9px 12px;font-weight:900;cursor:pointer;}',
    '.visitor-invite-reply{background:#e8000a;color:#fff;flex:1;}',
    '.visitor-invite-close{background:#2a2a2a;color:#fff;width:42px;}',
    '#chat-popover.open .visitor-invite-panel{display:flex;flex-direction:column;gap:10px;}',
    '#chat-popover.open .visitor-invite-panel .chat-msgs{min-height:260px;}'
  ].join('');
  document.head.appendChild(st);
}

function closeVisitorInviteToast(){
  const el = document.getElementById('visitor-invite-toast');
  if(el) el.remove();
}

window.descartarInvitacionVisitante = async () => {
  const invite = currentVisitorInvite;
  closeVisitorInviteToast();
  if(invite?.id){
    try{ sessionStorage.setItem('tomauno-visitor-invite-seen', invite.id); }catch(e){}
  }
  try{ await remove(ref(db,'tomauno/visitorInvites/'+PRESENCE_ID)); }catch(e){}
};

function renderVisitorInviteToast(invite){
  if(isAdminNotifier() || currentVisitorChatId) return;
  visitorInviteStyle();
  currentVisitorInvite = invite;
  closeVisitorInviteToast();
  const box = document.createElement('div');
  box.id = 'visitor-invite-toast';
  box.className = 'visitor-invite-toast';
  box.innerHTML =
    '<strong>Javier de Tomauno</strong>' +
    '<p>'+escHtml(invite.message || VISITOR_INVITE_DEFAULT)+'</p>' +
    '<div class="visitor-invite-actions">' +
      '<button class="visitor-invite-reply" onclick="window.abrirInvitacionVisitante()">Responder</button>' +
      '<button class="visitor-invite-close" title="Cerrar" onclick="window.descartarInvitacionVisitante()">x</button>' +
    '</div>';
  document.body.appendChild(box);
}

function initVisitorInviteListener(){
  try{
    onValue(ref(db,'tomauno/visitorInvites/'+PRESENCE_ID), snap => {
      if(isAdminNotifier() || currentVisitorChatId) return closeVisitorInviteToast();
      if(!snap.exists()) return;
      const invite = snap.val() || {};
      const now = Date.now();
      if(!invite.active || Number(invite.expiresAt || 0) < now){
        remove(ref(db,'tomauno/visitorInvites/'+PRESENCE_ID)).catch(()=>{});
        return closeVisitorInviteToast();
      }
      try{
        if(sessionStorage.getItem('tomauno-visitor-invite-seen') === invite.id) return;
      }catch(e){}
      renderVisitorInviteToast(invite);
    });
  }catch(e){}
}
initVisitorInviteListener();

window.abrirInvitacionVisitante = () => {
  const invite = currentVisitorInvite;
  ...
  setChatPopover(
    ...
    '<div class="chat-panel visitor-invite-panel"><div class="chat-msgs" id="chat-msgs">' +
      '<div class="chat-bubble admin"><div>'+chatLinkify(invite.message || VISITOR_INVITE_DEFAULT)+'</div><div class="chat-meta">Ahora</div></div>' +
    '</div>' +
    '<div class="chat-row"><input class="finput" id="chat-visitor-invite-text" placeholder="Responder a Javier..." onkeydown="if(event.key===\'Enter\')window.responderInvitacionVisitante()"/><button class="chat-send" onclick="window.responderInvitacionVisitante()">âžœ</button></div></div>'
  );
  setTimeout(()=>{ const inp=document.getElementById('chat-visitor-invite-text'); if(inp) inp.focus(); },80);
};

window.responderInvitacionVisitante = async () => {
  const invite = currentVisitorInvite;
  const inp = document.getElementById('chat-visitor-invite-text');
  ...
  const chatRef = await push(ref(db,'tomauno/chats'), {
    ...
    javierOnlineAt:now,
    visitorInvite:true,
    visitorPresenceId:PRESENCE_ID
  });
  currentVisitorChatId = chatRef.key;
  try{
    sessionStorage.setItem('tomauno-chat-id', currentVisitorChatId);
    sessionStorage.setItem('tomauno-chat-name', 'Visitante web');
    sessionStorage.setItem('tomauno-visitor-invite-seen', invite.id);
  }catch(e){}
  await push(ref(db,'tomauno/chats/'+currentVisitorChatId+'/messages'), {
    from:'admin',
    text:invite.message || VISITOR_INVITE_DEFAULT,
    time:chatTime(),
...
window.invitarVisitantesActivos = async () => {
  if(!isAdminNotifier()){ toast('ActivÃ¡ el modo admin para invitar visitantes'); return; }
  const visitors = activeVisitorPresenceEntries();
  if(!visitors.length){ toast('No hay visitantes activos para invitar'); return; }
  document.getElementById('mcontent').innerHTML =
    ...
    '<textarea class="finput" id="visitor-invite-message" style="min-height:130px;">'+escHtml(VISITOR_INVITE_DEFAULT)+'</textarea>' +
    '<button class="btn-main" onclick="window.enviarInvitacionVisitantes()">Enviar ahora</button>' +
    '<button class="btn-out" onclick="window.closeModal()">Cancelar</button>';
  openModal();
  setTimeout(()=>document.getElementById('visitor-invite-message')?.focus(),80);
};

window.enviarInvitacionVisitantes = async () => {
  const visitors = activeVisitorPresenceEntries();
  const message = String(document.getElementById('visitor-invite-message')?.value || '').trim();
  if(!message){ toast('EscribÃ­ el mensaje'); return; }
  if(!visitors.length){ closeModal(); toast('No hay visitantes activos'); return; }
  const now = Date.now();
  const inviteId = 'vis_' + now + '_' + Math.random().toString(36).slice(2,7);
  const updates = {};
  visitors.forEach(([id]) => {
    updates[id] = {id:inviteId, message, active:true, createdAt:now, expiresAt:now + 5 * 60 * 1000};
  });
  await update(ref(db,'tomauno/visitorInvites'), updates);
  closeModal();
...
document.addEventListener('keydown', ev => {
  if((ev.ctrlKey || ev.metaKey) && String(ev.key || '').toLowerCase() === 'm'){
    if(!isAdminNotifier()) return;
    ev.preventDefault();
    window.invitarVisitantesActivos();
  }
});
...
function abrirPanelChatsAdmin(){
  ...
  setChatPopover(
    ...
    '<button class="chat-filter" title="Invitar visitantes activos" onclick="window.invitarVisitantesActivos()">VIS</button>' +
