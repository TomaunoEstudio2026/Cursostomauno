...
window.enviarTicketPagoAlumno = (id) => {
  ...
  setTimeout(()=>drawTicketCanvas({alumno:i.nombre||'-', curso:cursoNombre, dni:i.dni||'-', wp:i.wp||'-', detallePedido:i.detallePedido||'', total, contratado, saldo, rows}), 50);
};
...
function drawTicketCanvas(data){
  ...
  if (data.detallePedido) { ctx.font='800 18px Arial';ctx.fillStyle='#777';ctx.fillText('DETALLE DEL PEDIDO',60,815);ctx.font='700 18px Arial';ctx.fillStyle='#222';wrapTicketText(data.detallePedido,60,845,390,24); }
...
window.exportarExcel = () => {
  ...
  const cols = ['Nro','Nombre','DNI','Edad','Curso',especialHeader,'Detalle pedido','Localidad','Instagram','WhatsApp','Fecha','Opciones','Total contratado','Total abonado','Saldo'];
  const rows = lista.map(([,i], idx) => {
    const cur = cursos[i.cursoId];
    const pinfo = getPagoAlumnoInfo(i, cur);
    return [idx+1, i.nombre||'', i.dni||'', i.edad||'', i.cursoTitulo || cur?.titulo || '', i.campoEspecialValor || '', i.detallePedido || '', i.localidad || '', i.ig||'', i.wp||'', i.fecha||'', resumenOpcionesElegidas(i), totalContratadoAlumno(i, cur), pinfo.monto, saldoAlumno(i, cur)];
  });
